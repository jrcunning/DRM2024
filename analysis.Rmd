...
# Subset just species with >=5 observations per Subregion
abund_spp2 <- df23ss %>%
  group_by(Subregion) %>%
  count(Species) %>%
  filter(n >= 5)
...
# Model bleaching severity by species and subregion
...
df23ss <- df23ss %>%
  mutate(BL = case_when(Bleaching == "BL" ~ 1, TRUE ~ 0))

mod <- glm(BL ~ Species * dhw, data = df23ss, family = "binomial")
emm <- emmeans(mod, specs = c("Species", "dhw"), at = list(dhw = seq(0,24,1)), type = "response")
as.tibble(emm) %>%
  ggplot(aes(x = dhw, y = prob, color = Species, group = Species)) +
  geom_line()# +
  #facet_wrap(~Species)

# binomial gam
mod <- gam(BL ~ s(dhw, by = Species), family = "binomial", 
           data = filter(df23ss, Species %in% c("MCAV", "SSID")))
emm <- emmeans(mod, specs = c("Species", "dhw"), at = list(dhw = seq(0,24,1)), type = "response")
as.tibble(emm) %>%
  ggplot(aes(x = dhw, y = prob, color = Species, group = Species)) +
  geom_line()

ggplot(filter(df23ss, Species %in% c("MCAV", "SSID"), !is.na(dhw)),
       aes(x = dhw.bin, fill = BL==1)) +
