...
# Overall percent of all corals observed that were partially bleached or worse
## (Raw data)
...
# Overall predicted probability of colonies being PB or worse during week 35 (Sept. 1st)
...
as.tibble(res) %>%
  ...
  ggplot(aes(x = response, y = Subregion, group = group)) +
  geom_point() +
  geom_path() +
  labs(x = "Proportion of corals bleached") +
  scale_x_continuous(limits = c(0,1), expand = c(0,0))
```

### Levels of bleaching observed for all corals with increasing DHWs
```{r bleaching_vs_dhw}
# Ordinal regression: Probability of each bleaching level with increasing DHW
m <- polr(Bleaching3 ~ dhw * Depth, data = df23, Hess = TRUE, model = TRUE)
...
df23.abund.ss2 <- df23.abund %>%
  mutate(sizeclass = case_when(Width <= 7 & (Old + Recent) <= 10 ~ "juv",
                               TRUE ~ "adult"))

mod <- glmer((Bleaching2 > 1) ~ Width * Species * dhw + (1|Site) + (1|Week), 
             family = "binomial", data = df23.abund.ss2, 
             nAGQ = 0, control = glmerControl(optimizer = "nloptwrap"))

#res <- emmeans(mod, specs = "Width", by = "Species", at = list(dhw = 15))
res <- emtrends(mod, specs = c("Width", "dhw"), var = "Width", by = "Species", at = list(dhw = c(10, 15, 20)))
#sig <- as.tibble(rbind(contrast(res, "pairwise"))) %>%
  #filter(p.value < 0.05)
sig <- as.tibble(res) %>%
  filter(asymp.LCL > 0)

# as.tibble(emmeans(mod, specs = "sizeclass", by = "Species", 
#                   at = list(dhw = 15), type = "response")) %>%
as.tibble(emmeans(mod, specs = c("Width", "dhw"), by = "Species", at = list(Width = seq(5, 100, 1),
                                                                  dhw = c(10, 15, 20)), 
                  type = "response")) %>%
  filter(Species %in% sig$Species) %>%
  #right_join(select(sig, dhw, Species)) %>%
  #ggplot(aes(x = sizeclass, y = response)) +
  ggplot(aes(x = Width, y = response, color = factor(dhw), group = dhw)) +
...
# Get subset of data
dfdf <- df23 %>%
  group_by(Species, Site, dhw, Depth, Week, Longitude, Latitude) %>%
