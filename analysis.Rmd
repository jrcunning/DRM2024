...
# Spatial differences in bleaching
...
```
![](output/maps_multipanel.png)
```{r}
# Try random forests to find outlier sites?
library(randomForest)
data <- df23.abund %>%
  select(Bleaching3, Species, Depth, dhw, Date, Week, Width, Habitat, Zone, Site) %>%
  droplevels()

set.seed(222)
ind <- sample(2, nrow(data), replace = TRUE, prob = c(0.7, 0.3))
train <- as.data.frame(data[ind==1,])
test <- as.data.frame(data[ind==2,])

rf <- randomForest(Bleaching3~Species+Depth+dhw+Date, data = data, ntree = 10, do.trace = TRUE, proximity=TRUE)
print(rf)

# p1 <- predict(rf, train)
# confusionMatrix(p1, train$Bleaching3)

out <- as_tibble(bind_cols(train, pred = rf$predicted))
out

out %>%
  filter(pred == "Bleached", Bleaching3 == "Healthy")
