---
title: "Untitled"
output: html_document
date: "2024-04-30"
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
```

```{r}
library(lme4)
library(emmeans)
library(ggridges)
library(gamm4)
library(MASS)
library(tidyverse)
```

# Import data

```{r}
# Import data
df <- read_csv("data/drm/export.csv") %>%
  mutate(across(where(is.character), as_factor),
         Date = as_date(as.character(Date), format = "%m/%d/%Y"))

# Get day of the year number, week, and month for some model fitting
df <- df %>%
  mutate(Day = yday(Date),
         Week = factor(week(Date)),
         Month = factor(month(Date)))

# Order Subregions from south to north
df <- df %>%
  mutate(Subregion = fct_relevel(Subregion, c("Tortugas--Dry Tortugas NP", "Marquesas", "Lower Keys", 
                                              "Middle Keys", "Mid-Upper Keys Transition", "Upper Keys",
                                              "Biscayne", "Broward-Miami", "South Palm Beach", 
                                              "North Palm Beach", "Martin")))

# Order Species alphabetically
df <- df %>%
  mutate(Species = factor(Species, levels = sort(levels(df$Species))))

# Create ordinal variables and factors for bleaching severity
df <- df %>%
  mutate(Bleaching2 = case_when((is.na(Bleaching) & !grepl("TRS", `Other Conditions`)) ~ 0,
                                Bleaching == "P" ~ 1,
                                Bleaching == "PB" ~ 2,
                                Bleaching == "BL" ~ 3,
                                ((Old + Recent) >= 100 & grepl("TRS", `Other Conditions`)) ~ 4),
         Bleaching3 = factor(case_match(Bleaching2, 
                                 0 ~ "Healthy",
                                 1 ~ "Pale",
                                 2 ~ "Partially Bleached",
                                 3 ~ "Bleached",
                                 4 ~ "Dead"),
                             levels = c("Healthy", "Pale", "Partially Bleached", "Bleached", "Dead")))

#### There are 45 corals listed with "TRS" but are blank for "Bleaching"... talk to Jennifer about these?
#### For now, remove these from analysis
df <- df %>%
  filter(!is.na(Bleaching2))

# Subset just 2023 surveys
df23 <- filter(df, year(Date) == "2023")

# Import 2023 DHW data
dhw <- read_csv("data/dhw/dhw_processed.csv") %>%
  mutate(across(where(is.character), as_factor)) %>%
  rename(Date = date) %>%
  dplyr::select(Site, Date, dhw)

# Get DHW values for each DRM survey site on date surveyed
df23 <- left_join(df23, dhw, by = c("Site", "Date"))

# Create DHW bins
df23 <- df23 %>%
  mutate(dhw.bin = cut(dhw, breaks = c(0,4,8,12,16,20,24)))
```

# Survey information

```{r}
# How many total corals surveyed of each species?
sppcounts <- df23 %>%
  count(Species) %>%
  arrange(-n)

# How many sites per Subregion?
# How are reef Zones diff across Subregions?
df23 %>%
  distinct(Subregion, Site, Zone) %>%
  group_by(Subregion) %>%
  ggplot(aes(x = Subregion, fill = Zone)) +
  geom_bar(position = "stack") +
  scale_y_continuous(breaks = seq(0, 100, 10)) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# How were 2023 surveys distributed across time and space?
surveys <- df23 %>%
  distinct(Subregion, Date, Site, dhw)

ggplot(surveys, aes(x = Date, y = Subregion, fill = Subregion)) +
  geom_density_ridges(alpha = 0.6, stat = "binline", bins = 20) +
  theme(legend.position = "none")

# How were surveys distributed across DHW accumulation?
ggplot(surveys, aes(x = dhw, y = Subregion, fill = Subregion)) +
  geom_density_ridges(alpha = 0.6, stat = "binline", bins = 20) +
  theme(legend.position = "none")
```

# Bleaching

```{r}
# Did overall bleaching severity change over time?

## Fit GAMM for overall bleaching level over time within each Subregion
mod <- gam(Bleaching2 ~ s(Day, by = Subregion), data = df23)
res <- emmeans(mod, specs = "Day", by = "Subregion", at = list(Day = seq(167, 318, 1)))
# Plot 
df23 %>%
  distinct(Subregion, Day, Date) %>%
  arrange(Subregion, Date) %>%
  left_join(as.tibble(res)) %>%
  mutate(Date = as.Date(Day, origin = as.Date("2024-01-01"))) %>%
  ggplot(aes(x = Date, y = emmean, color = Subregion, group = Subregion)) +
  geom_line() +
  ylim(0, 3)

```

# Subset data for bleaching analysis
```{r}
df23ss <- df23 #%>%
  # Subset mid-August to end of September? (stable peak of bleaching to allow comparing across Subregions, e.g. Marquesas was in Oct. only)
  #filter(Date >= "2023-08-15" & Date <= "2023-09-30")

# Filter out dead corals from causes OTHER than bleaching ("TRS")
df23ss <- df23ss %>%
  filter(((Old + Recent) < 100 | grepl("TRS", `Other Conditions`)))

# Subset just species with >=3 observations per Subregion
abund_spp2 <- df23ss %>%
  group_by(Subregion) %>%
  count(Species) %>%
  filter(n >= 3)

df23ss <- df23ss %>%
  right_join(abund_spp2) %>%               # Apply filter
  mutate(Species = droplevels(Species))
```

# - [ ] Bleaching severity by species (in relation to location and DHW). which species more/less bleached
```{r}
# Average bleaching severity level (0-4) of each species in each subregion
mod <- lmerTest::lmer(Bleaching2 ~ Species * Subregion + (1|Site) + (1|Month), data = df23ss)
anova(mod)

emm <- emmeans(mod, specs = "Species", by = "Subregion")

bleaching_by_region_all <- as.tibble(emm) %>%
  ggplot(aes(x = Subregion, y = emmean, color = Species, group = Species)) +
  geom_point() +
  geom_line() +
  facet_wrap(~Species) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

bleaching_by_region_all %+% filter(as.tibble(emm), Species %in% sppcounts$Species[which(sppcounts$n > 75)])
```

```{r}
# Probability of each bleaching level by species as a function of DHW (ordinal regression)
#https://tdunn.ca/posts/2020-03-15-ordinal-regression-in-r-part-1/


# Subset corals further for species with enough data to model an interaction effect with Species*dhw
df23ss2 <- df23ss %>%
  filter(Species %in% sppcounts$Species[which(sppcounts$n > 75)])

# Probability of each bleaching level by species as a function of DHW
m <- polr(Bleaching3 ~ Species * dhw, data = df23ss2, Hess = TRUE, model = TRUE)

res <- emmeans(m, specs = c("dhw", "Bleaching3"), by = "Species", mode = "prob", at = list(dhw = seq(0, 24, 1)))

bleachcat_by_dhw <- as.tibble(res) %>%
  ggplot(aes(x = dhw, y = prob, color = Bleaching3)) +
  geom_line() +
  facet_wrap(~Species) +
  xlim(0,24)

##### GOOD
bleachcat_by_dhw %+% filter(as.tibble(res), Species %in% sppcounts$Species[which(sppcounts$n > 75)])
#####

thresh <- as.tibble(res) %>%
  filter(Species %in% sppcounts$Species[which(sppcounts$n > 100)]) %>%
  filter(Bleaching3 == "Bleached") %>%
  group_by(Species) %>%
  summarize(thresh = min(dhw[prob > 0.5])) %>%
  mutate(Species = fct_reorder(Species, thresh))

##### BETTER
as.tibble(res) %>%
  filter(Species %in% sppcounts$Species[which(sppcounts$n > 100)]) %>%
  mutate(Species = factor(Species, levels = levels(thresh$Species))) %>%
  ggplot(aes(x = dhw, y = prob, color = Bleaching3)) +
  geom_line() + 
  facet_wrap(~Species) +
  geom_vline(data = thresh, aes(xintercept = thresh), lty = 2, lwd = 0.25)
#####

## Calculate proportion of colonies fully bleached or Dead at diff. DHW levels
res2 <- emmeans(m, specs = c("dhw", "Bleaching3"), by = "Species", mode = "prob", at = list(dhw = c(10, 20)))

BlDead <- as.tibble(res2) %>%
  filter(Species %in% sppcounts$Species[which(sppcounts$n > 75)],
         Bleaching3 %in% c("Bleached", "Dead")) %>%
  group_by(Species, dhw) %>%
  summarize(BlDead = sum(prob))

ranks <- BlDead %>%
  group_by(Species) %>%
  summarize(meanBlDead = BlDead[dhw==20]) %>%
  #filter(dhw == 10) %>%
  arrange(-meanBlDead) %>%
  mutate(Species = fct_reorder(Species, -meanBlDead))

BlDead %>%
  mutate(Species = factor(Species, levels = levels(ranks$Species))) %>%
  ggplot(aes(x = Species, y = BlDead, color = factor(dhw))) +
  geom_point() +
  #geom_errorbar(aes(ymin = asymp.LCL, ymax = asymp.UCL), width = 0.1) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))

# Show just some spp of interest
as.tibble(res) %>%
  filter(Species %in% c("AAGA", "ACER", "CNAT", "DLAB", "DSTO", "OFAV", "MCAV", "SSID")) %>%
  filter(Bleaching3 == "Bleached") %>%
  ggplot(aes(x = dhw, y = prob, color = Species)) +
  geom_line()

# Look at data for individual species
df23ss2 %>%
  filter(Species == "ACER") %>%
  ggplot(aes(x = dhw.bin, fill = Bleaching3)) +
  geom_bar()

# Where was healthy ACER observed with 8-12 DHW?
test <- df23ss2 %>%
  filter(Species == "ACER", Bleaching3 == "Healthy", dhw > 8)
View(test)
y1069 <- df23ss2 %>% filter(Site == "Y1069")
View(y1069)
```

```{r}
ggplot(df23ss, aes(x = dhw, y = Bleaching2, color = Species)) +
  geom_point(alpha = 0.1) +
  facet_wrap(~Species)

# Bleaching as a function of DHW bin for each Species
mod <- lmerTest::lmer(Bleaching2 ~ Species * dhw.bin + (1|Site), data = df23ss)
anova(mod)

emm <- emmeans(mod, specs = c("Species", "dhw.bin"), at = list(dhw.bin = levels(df23ss$dhw.bin)))

as.tibble(emm) %>%
  ggplot(aes(x = dhw.bin, y = emmean, color = Species, group = Species)) +
  geom_point() +
  geom_line() +
  #facet_wrap(~Species) +
  ylim(0, 4)



# Need to try some binomial modeling with BL as response (1=BL, 0=notBL) to model proportion of colonies bleached at various locations and dhw levels....

mod <- glm((Bleaching2 >= 3) ~ Species * dhw, data = df23ss, family = "binomial")
emm <- emmeans(mod, specs = c("Species", "dhw"), at = list(dhw = seq(0,24,1)), mode = "prob", type = "response")
as.tibble(emm) %>%
  filter(Species %in% sppcounts$Species[which(sppcounts$n > 75)]) %>%
  ggplot(aes(x = dhw, y = response, color = Species, group = Species)) +
  geom_line()# +
  #facet_wrap(~Species)
```




```{r}
# - [ ] Variability in bleaching among individuals within a species 
### which spp have the most variability among individuals?

var <- df23ss %>%
  group_by(Site, Latitude, Longitude, Species, Subregion, Zone) %>%
  summarize(n = n(),
            bleachvar = var(Bleaching2),
            max = max(Bleaching2)) %>%
  filter(n >= 10, max == 3, bleachvar > 0)

ggplot(var, aes(x = Species, y = bleachvar)) +
  geom_violin(scale = "width", draw_quantiles = 0.5) +
  geom_jitter(aes(color = Subregion), width = 0.2)

## where do tol. and sens. individuals co-occur?

var %>% filter(Species == "MCAV") %>%
  ggplot(aes(x = Longitude, y = Latitude)) +
  geom_point(aes(color = bleachvar))


#### Barplots of bleaching categories for each species
filter(df23ss, Species == "SSID") %>%
  ggplot(aes(x = Week, fill = factor(Bleaching2))) +
  geom_bar(position = "fill") +
  facet_grid(Subregion~"")
```

```{r}
#  - [ ] Where are heat-tolerant individuals? i.e., most non-bleached individuals rel. to DHW
n_distinct(df23ss$Site)
n_distinct(summ$Site)

summ <- df23ss %>%
  group_by(Site, Bleaching3, dhw, Depth) %>%
  count() %>%
  group_by(Site, dhw, Depth) %>%
  summarize(hp = (n[Bleaching3 == "Healthy"] + n[Bleaching3 == "Pale"]) / sum(n))

ggplot(summ, aes(x = dhw, y = hp)) +
  geom_point()

# Model percent of healthy/pale colonies as function of DHW and Depth
mod <- lm(hp ~ dhw * Depth, data = summ)
anova(mod)
summary(mod)
# Find Sites with biggest positive residuals (more healthy/pale colonies than expected)
bind_cols(summ, broom::augment(mod)) %>%
  arrange(-.std.resid)
# Y1368 and Y2331 have largest positive residual for number of healthy/pale colonies
# Where are these?
hi <- df23 %>% filter(Site %in% c("Y1368", "Y3042", "Y3034", "Y1145", "Y1084")) %>%
  distinct(Site, Latitude, Longitude)
View(hi)
# Newfound Harbor is #1. Others are Looe, Molasses, Miami Beach, and FLL... kinda random...


# Need to do this on the colony-level...
df23ss2 <- df23ss %>% 
  #filter(Subregion %in% c("Lower Keys", "Upper Keys", "Biscayne")) %>%
  #filter(Species %in% c("MCAV", "AAGA", "PPOR")) %>%
  mutate(Site = droplevels(Site)) %>%
  mutate(BL = Bleaching2 > 2)
mod <- glmer(BL ~ dhw * Depth + (dhw|Species) + (Depth|Species) + (1|Date), family = "binomial", data = df23ss2,
             nAGQ = 0, control = glmerControl(optimizer = "nloptwrap"))

anova(mod)
plot(mod)
test <- tibble(df23ss2, .resid = residuals(mod, type = "response"))
test2 <- test %>% group_by(Site, Longitude, Latitude) %>%
  summarize(meanResid = mean(.resid)) %>%
  arrange(meanResid)
# negative values indicate less bleaching than predicted based on dhw, depth, and species...

# Plot sites that bleached more/less than expected
myPlot <- ggplot(test2, aes(x = Longitude, y = Latitude, color = meanResid)) +
  geom_point(alpha = 0.4) +
  scale_color_gradient2()

myPlot + xlim(-82, -81) + ylim(24.2, 24.8)
myPlot + xlim(-81, -80) + ylim(24.5, 25.5)
myPlot + xlim(-80.3, -80) + ylim(25.5, 27) +
  geom_hline(yintercept = c(25.7588, 26.0923, 26.257518, 26.54476, 26.77238))


myPlot +
  geom_hline(yintercept = c(25.7588, 26.0923, 26.54476, 26.77238))




#
hi <- test %>% filter(Site %in% c("Y3042"))
View(hi)

hi2 <- df23 %>% filter(Site %in% c("Y3042"))
View(hi2)
```


```{r}
# - [ ] Are juveniles doing better (less bleaching) than larger adult colonies for any species?

# Model bleaching severity by species and size class
df23ss <- df23ss %>%
  mutate(sizeclass = case_when(Width <= 8 & (Old + Recent) <= 10 ~ "juv",
                               TRUE ~ "adult"))
mod <- lmerTest::lmer(Bleaching2 ~ sizeclass * Species + Subregion + (1|Site) + (1|Month), 
                      data = filter(df23ss, !Subregion %in% c("Broward-Miami", "South Palm Beach",
                                                              "North Palm Beach", "Martin")))
anova(mod)

emm <- emmeans(mod, specs = c("Species", "sizeclass"))
as.tibble(emm) %>%
  ggplot(aes(x = sizeclass, y = emmean)) +
  facet_wrap(~Species, scales = "free") +
  geom_point() +
  geom_errorbar(aes(ymin = asymp.LCL, ymax = asymp.UCL))

stats <- rbind(contrast(emm, "pairwise", by = "Species")) %>%
  as.tibble() %>%
  arrange(p.value) %>%
  print(n = nrow(.))

sigspp <- stats %>%
  filter(p.value < 0.05)

as.tibble(emm) %>%
  filter(Species %in% sigspp$Species) %>%
  ggplot(aes(x = sizeclass, y = emmean)) +
  facet_wrap(~Species) +
  geom_point() +
  geom_errorbar(aes(ymin = asymp.LCL, ymax = asymp.UCL))
```
    

    
