...
# Calc DHWs to 50% bleaching for each species
test <- as.tibble(res) %>%
  group_by(Species, dhw) %>%
  summarize(PBorB = prob[Bleaching3 == "Partially Bleached"] + prob[Bleaching3 == "Bleached"]) %>%
  group_by(Species) %>%
  summarize(thresh25 = min(dhw[PBorB > 0.25]),
            thresh50 = min(dhw[PBorB > 0.5])) %>%
  mutate(Species = fct_reorder(Species, thresh25)) %>%
  pivot_longer(cols = c(2, 3))

ggplot(test, aes(x = Species, y = value, color = name)) +
  geom_point()
