...
drc.ord %>%
  left_join(span1090) %>%
  left_join(readxl::read_xlsx("data/species_metadata.xlsx")) %>%
  ggplot(aes(x = n_symbiont_genera, y = span)) + geom_point()
```
...
(c1 <- coef(mod)[2])
## 0.7308 DHW's per year --> 0.57°C/decade
c1 * 0.0777 * 10

# Change over time considering just 2014-2023
ed50conf %>%
  filter(year == "2023-2014") %>%
  mutate(across(c("ed50", "se", "lower", "upper"), ~ . * 0.0777 / 9 * 10, .names = "{.col}.degC.dec"))

# 0.67°C/decade [95% C.I. = 0.56-0.72°C/decade]
```
