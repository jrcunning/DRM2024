...
### With the glmer above, cannot use dose.p function to get ED50. Try glm
...
df23f <- df23.abund %>%
  ...
  summarize(BL = sum(Bleaching2 > 1),
            ...
            n = n()) %>%
  drop_na()
m4 <- drm(pctNB ~ dhw, curveid = Species,
          weights = n,
          fct = LL.5(), 
          data = df23f)
m4res <- broom::tidy(m4)
...
m4res %>%
  filter(term == "ED50") %>%
  mutate(curve = fct_reorder(curve, estimate)) %>%
  ggplot(aes(x = estimate, y = curve)) +
  geom_point()

out <- broom::augment(m4, df23f)
