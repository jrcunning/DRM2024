...
dhw.bleach.plot <- ggplot(as.tibble(dhw.res2), aes(x = dhw, y = prob, group = year)) +
  ...
  geom_line(aes(color = prob), lwd = 1) +
...
dhw.bleach.plot +
  ...
  # geom_errorbar(data = slice(ed50conf, 1:3), 
  #              aes(x = ed50, y = 0.01, xmin = ed50-se, xmax = ed50+se), 
  #              width = 0, lwd = 2, alpha = 0.3)
...
ggplot(net, aes(x = estimate, y = degC.dec)) + 
  geom_errorbar(aes(ymin = degC.dec - degC.dec.se * 0.0777, ymax = degC.dec + degC.dec.se * 0.0777)) +
