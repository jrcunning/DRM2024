...
test <- as.tibble(res2) %>%
  dplyr::select(Species, dhw, response) %>%
  pivot_wider(id_cols = "Species", names_from = dhw, values_from = response)
clust <- hclust(dist(test[,-1]))
plot(clust)
cutree(clust, k = 5)
grps <- test %>%
  dplyr::select(Species) %>%
  mutate(clust = cutree(clust, k = 5))
as.tibble(res2) %>%
  left_join(grps) %>%
  ggplot(aes(x = dhw, y = response, color = Species, group = Species)) +
  geom_line() +
  geom_label_repel(data = grps, aes(x = 5, y = 0.5, label = Species), max.overlaps = 100) +
  facet_wrap(~clust)
...
### With the glmer above, cannot get ED50, and cannot constrain to all healthy at DHW=0. Try DRC?
library(drc)
df23f <- df23.abund %>%
...
# Fit model to each coral species separately, get parameters, fitted values, and residuals
initmods <- df23f %>%
  nest(data = c(Site, Subregion, Week2, dhw, BL, NB, pctNB, n)) %>%
  ...
  mutate(pars = map(ll3, tidy),
         pred = map(ll3, ~augment(., newdata = data.frame(dhw = seq(0,23,1)))),
         dhw25 = map(ll3, ~as_tibble(ED(., 0.25, type = "absolute"))),
         dhw50 = map(ll3, ~as_tibble(ED(., 0.50, type = "absolute"))),
         dhw75 = map(ll3, ~as_tibble(ED(., 0.75, type = "absolute"))))

# Extract ed50 parameter values from model fits
outt <- initmods %>% 
  dplyr::select(Species, dhw50) %>%
  unnest(dhw50)
...
ggplot(vals, aes(x = dhw, y = .fitted)) +
  geom_line() +
  facet_wrap(~Species)



# Single DRC with curveid = Species?
# Gets an error if DLAB is included
df23ff <- filter(df23f, Species ) %>%
  droplevels()
df23f <- droplevels(df23f)
m <- drm(pctNB ~ dhw, 
    data = df23f, 
    curveid = Species,
    weights = n,
    ...
    lowerl = c(rep(0, 22), rep(0, 22)),
    control = drmc(relTol = 1e-7))
# Stats on ED50s
compParm(m, "ED50", operator = "-")
plot(m)

nd <- expand.grid(Species = levels(df23f$Species), dhw = seq(0, 23, 1))
pred <- predict(m, newdata = nd)
myres <- as_tibble(bind_cols(nd, pred = pred))
# Cluster
test <- as.tibble(myres) %>%
  pivot_wider(id_cols = "Species", names_from = dhw, values_from = pred)
clust <- hclust(dist(test[,-1]))
plot(clust)
cutree(clust, k = 4)
grps <- test %>%
  dplyr::select(Species) %>%
  mutate(clust = cutree(clust, k = 6))


myres %>%
  left_join(grps) %>%
  ggplot(aes(x = dhw, y = pred)) +
...
myp <- ggplot(data.frame(0), aes(x = dhw, y = prob, color = Species)) +
         geom_line() +
         facet_wrap(~Species)
...
ggplot(allres, aes(x = dhw, y = prob, color = Species)) +
         geom_line(aes(linetype = method)) +
         facet_wrap(~Species)
ggplot(drc.res, aes(x = dhw, y = prob, color = Species)) +
  geom_line()
...
```{r, eval = FALSE}
##^^ THis is pretty good, but can I get medrm to work? 

mell3 <- function(data) {
  medrm(pctNB ~ dhw, data = data,
      fct = LL.3(fixed = c(NA, 1, NA)), 
      random = b + e ~ 1|Week2/Subregion,
      control=nlmeControl(tolerance=0.1, pnlsTol=1))
  }
trymell3 <- possibly(mell3, otherwise = NULL)
aagamell3 <- trymell3(filter(df23f, Species == "PFUR"))
aagall3 <- tryll3(filter(df23f, Species == "PFUR"))
plot(aagall3)
plot(aagamell3)
aagamell3predvals <- predict(aagamell3, newdata = data.frame(dhw = seq(0,23,1)), level = 0)
aagall3predvals <- predict(aagall3, newdata = data.frame(dhw = seq(0,23,1)), level = 0)
plot(aagall3, xlim = c(0, 23))
plot(NA, xlim = c(0,23), ylim = c(0,1))
lines(seq(0,23,1), aagamell3predvals)
lines(seq(0,23,1), aagall3predvals)

augment(aagall3, filter(df23f, Species == "AAGA")) %>%
  ggplot(aes(x = dhw, y = .fitted)) + geom_point()
predict(aagall3, newdata = data.frame(dhw = seq(0, 23, 1)))


medrm(pctNB ~ dhw, data = filter(df23f, Species == "OFAV"),
      fct = LL.4(fixed = c(NA, NA, 1, NA)), 
      random = b + c + e ~ 1|Week2/Subregion,
      control=nlmeControl(tolerance=0.1, pnlsTol=1))

initmods <- df23f %>%
  nest(data = c(Site, Subregion, Week2, dhw, BL, NB, pctNB, n)) %>%
  # Fit the model to each coral
  mutate(ll3 = map(data, trymell3))
plot(initmods$ll3[[1]], xlim = c(0,22))

nd <- data.frame(dhw = seq(0, 23, 1), Week2 = "(33,35]", Subregion = "Middle Keys")

pred = predict(initmods$ll3[[17]], newdata = nd)
plot(seq(0, 23, 1), pred, ylim = c(0, 1))

plot(initmods$ll3[[17]])


  mymods <- function(n) {
    medrm(pctNB ~ dhw, data = df23f, 
          curveid = b + e ~ Species,
          fct = LL.3(fixed = c(NA, 1, NA)), 
      random = b + e ~ 1|Week2/Subregion,
      start = rep(1, n),
      control=nlmeControl(tolerance=0.1, pnlsTol=1))}
  trymymods <- possibly(mymods, otherwise = NULL)
  
# Try mell3 with curveID = Species
out <- tibble(n = 1:500) %>%
  mutate(mod = map(n, ~trymymods(.)))
for (n in 1:100) {
  mod <- trymymods(n)
}
