...
#                        family = "binomial", data = allg)
###^^ This model works
dhw.bin.mod <- glmer(cbind(BL, NB) ~ dhw.bin * year + year:Week2 + (dhw.bin+year|Species) + (year|Subregion),  
...
# OK now what if use dhw instead of dhw.bin
...
dhw.res <- emmeans(dhw.mod, specs = c("dhw", "year", "Week2"), type = "response",
                   at = list(dhw = seq(0, 23, 0.1)), rg.limit = 12000)
# Get emmeans for just the specific 2-week windows of interest in each year (corresp. to max. bleaching)
...
# # OK now what if we use DRC? -- cannot use drc because not high enough dhw's and bleaching in 2014/2015 to fit a full curve
# allg <- allg %>% mutate(n = BL + NB, pctNB = NB / n)
# m5 <- drm(pctNB ~ dhw, data = allg, curveid = year, weights = n, type = "binomial",
#          logDose = NULL, fct = LL.3(
#            names = c("hill", "max", "ED50"),
#            ## Max parameter is fixed to 1
#            fixed = c(NA, 1, NA)),
#     # Hill parameter must be positive (must decrease with incr. dhw)
#     lowerl = c(rep(0, 3), rep(0, 3)),
#     control = drmc(relTol = 1e-7))
# nd <- expand.grid(year = c("2014", "2015", "2023"), dhw = seq(0, 23, 0.1))
# pred <- predict(m5, newdata = nd)
# drc.res <- as_tibble(bind_cols(nd, prob = pred))
# drc.ord <- ED(m5, 0.5, type = "absolute") %>%
#   as_tibble() %>%
#   mutate(Species = levels(df23f$Species),
#          Species = fct_reorder(Species, Estimate))
# ggplot(as.tibble(drc.res), aes(x = dhw, y = prob, color = year, group = year)) +
#   #geom_point() +
...
# Fit model
mod <- glm(Bleaching2 > 1 ~ Species * Width2 * dhw + Week2 + (1|Subregion), 
