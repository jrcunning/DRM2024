...
### Reduced bleaching sensitivity in 2023 vs. 2014 and 2015

```{r bleaching_by_dhw.bin_across_years}
# Filter to just pre-DHW peak?
# all0.prepeak <- all0 %>%
#   filter((year == "2014" & Date <= as.Date("2014-09-21")) |
#          (year == "2015" & Date <= as.Date("2015-09-20")) |
#          (year == "2022" & Date <= as.Date("2022-09-27")) |
#          (year == "2023" & Date <= as.Date("2023-09-03")))
...
#   filter(n() > 3)
# Try another filter just getting rid of poorly sampled species
spp_to_include <- all0 %>%
  count(Species, year) %>%
  group_by(Species) %>%
  filter(min(n) >= 20)
...
# Model differences in bleaching severity between the three years as a function of DHW bin
dhw.bin.mod <- glmer(cbind(BL, NB) ~ dhw.bin * year + year:Week2 +
                       (dhw.bin+year|Species) + (year|Subregion),  
                     family = "binomial", data = allg, verbose = TRUE,
                     nAGQ=0, control=glmerControl(optimizer = "nloptwrap"))
## Best formula seems to be: cbind(BL, NB) ~ dhw.bin * year + year:Week2 + (dhw.bin+year|Species) + (year|Subregion)
...
# Plot
dhw.bin.bleach.plot <- as.tibble(dhw.bin.res2) %>%
  ggplot(aes(x = dhw.bin, y = prob, shape = year, group = year)) +
  geom_ribbon(aes(ymin = prob - SE, ymax = prob + SE), lwd = 0, alpha = 0.1) +
  geom_line() +
  geom_point(aes(fill = prob), size = 4) +
  scale_x_discrete(labels = c("0-3", "3-6", "6-9", "9-12", "12-15", "15-18", "18-22")) +
...
#rbind(pairs(dhw.bin.res2, by = "dhw.bin"), adjust = "none")
...
# Combine plots from both approaches for comparison
bleachplots <- cowplot::plot_grid(dhw.bin.bleach.plot, dhw.bleach.plot, ncol = 2)
ggsave(filename = "output/bleaching_years.png", plot = bleachplots, width =150, height = 70, units = "mm")
```
![](./output/bleaching_years.png)
```{r}
# Backcalculate increase in tolerance based on 50% bleaching
bleach50 <- as.tibble(dhw.res2) %>%
  group_by(year) %>%
  slice(which.min(abs(prob - 0.5)))

# In 2014: 3.9 DHW. In 2023: 11.2 DHW. (In 2015, did not reach 50% bleaching)
# 11.7 - 3.9 = 7.8 DHW increase

threshy <- bleach50 %>%
  ungroup() %>%
  mutate(diffdhw = dhw - dhw[year == "2014"],
         diffthresh = diffdhw * 0.077)

ggplot(threshy, aes(x = as.numeric(as.character(year)), y = diffthresh)) + 
  geom_point() + geom_smooth(method = "lm")

mod <- lm(diffthresh ~ as.numeric(as.character(year)), data = threshy)
summary(mod)
```

## Species-specific bleaching severity by DHWs in 2023 vs. 2014 and 2015

```{r species_bleaching_by_year_dhw.bin}
...
dhw.bin.sp.mod <- glmer(cbind(BL, NB) ~ year * Species * dhw.bin + year:Week2 + (year|Subregion), 
...
dhw.sp.res <- emmeans(dhw.sp.mod, specs = c("Species", "dhw", "year", "Week2"), type = "response",
                   at = list(dhw = seq(0, 23, 0.1)), rg.limit = 200000, level = 0.84)
...
spp_thresh <- as.tibble(dhw.sp.res22) %>%
  ...
  group_by(Species) %>%
  mutate(diffC = (dhw - dhw[year == "2014"]) * 0.077)
...
```{r last_try_drc_fitting_diff_years_by_species}
# Here we fit drc for one species with curves for different years. Best result is allowing only the ED50 to vary by year with others fit across all years, or use contraints on parameters. In any case, the forms seem to inflexible to fit well to 2014 and 2015 data, which do not even reach a 50% bleaching response for many of the species...
# one more try of drc?
allg2 <- allg %>%
  mutate(n = BL + NB,
         pctNB = NB / (NB + BL)) %>%
...
# More pars = less estimable standard errors on ED50... depends how important ED50 inference is
m3 <- drm(pctNB ~ dhw, data = allg2, curveid = year, weights = n, type = "binomial",
         logDose = NULL, fct = LL.3(
           names = c("hill", "max", "ED50"),
           ## Max parameter is fixed to 1
           fixed = c(NA, 1, NA)),
         pmodels = data.frame(year, year, year),
    # Hill parameter must be positive (must decrease with incr. dhw)
    lowerl = c(rep(1, 3), rep(0, 3)),
    control = drmc(relTol = 1e-7))
nd <- expand.grid(year = unique(allg2$year), dhw = seq(0, 23, 0.1))
pred <- predict(m3, newdata = nd)
drc.res <- as_tibble(bind_cols(nd, prob = pred))
drc.ord <- ED(m3, 0.5, type = "absolute")

drc.res %>% #filter(year == "2023") %>%
ggplot(aes(x = dhw, y = prob, color = year, group = year)) + geom_line()
spp_thresh %>% filter(Species == "PAST")
```
...
```{r}
myfun <- right_join(mydens, spp_thresh, by = c("Species", "year"))
...
myfun %>%
  ...
  geom_errorbar(aes(xmin = rate - SE, xmax = rate + SE)) +
