...
```{r within_spp_bleaching_variability, fig.width = 10, fig.height = 7}
...
ggsave(filename = "output/medrangeplot.png", plot = medrangeplot, width = 90, height = 90, units = "mm")

cowplot::plot_grid(drc.res.ridges, medrangeplot, ncol = 2, rel_widths = c(0.5, 0.5))
```
...
(dhw.bleach.plot <- ggplot(as.tibble(dhw.res2), aes(x = dhw, y = prob, group = year)) +
  ...
  geom_segment(data = slice(ed50conf, 1:3),
               aes(x = ed50, xend = ed50, y = 0.5, yend = 0), lty = 2, lwd = 0.25) +
  geom_segment(aes(x = -1, xend = 11.5, y = 0.5, yend = 0.5), lty = 2, lwd = 0.2) +
  # geom_text(data = slice(ed50conf, 1:3), 
  #           aes(x = ed50, y = 0.54, label = year),
  #           hjust = 1, angle = 40) +
  geom_text(data = slice(ed50conf, 1:3), 
            aes(x = c(8,9.5,20.5), y = c(0.8, 0.65, 0.97), label = year)) +
  scale_x_continuous(limits = c(-1, 23), breaks = seq(0, 24, 4), expand = c(0, 0)) +
...
plot1 <- ed50conf %>%
  slice(1:3) %>%
  ggplot(aes(x = as.numeric(year), y = ed50)) + geom_point() +
  geom_smooth(method = "lm", se = F) +
  geom_path(data = filter(ed50conf, year %in% c("2014", "2023"))) +
  theme_classic() +
...
(c1 <- coef(mod)[2])
## 0.7123 DHW's per year --> 0.55°C/decade
c1 * 0.0777 * 10

# Change over time considering just 2014-2023
(c2 <- ed50conf %>% filter(year == "2023-2014") %>% pull(ed50) / 9)
## 0.8444 DHW's per year --> 0.66°C/decade
c2 * 0.0777 * 10
```

## Species - ED50s 2014-15-23

```{r species_bleaching_by_year_dhw, fig.width = 7, fig.height = 9}
...
## Species - ∆ED50, ∆°C
```{r sp.changeED50, fig.width = 7, fig.height = 9}
# Convert ∆ED50s to degC
sp.degC <- sp.ed50conf %>%
  filter(year %in% c("2015-2014", "2023-2014")) %>%
  mutate(across(c("ed50", "se", "lower", "upper"), ~ . * 0.077, .names = "{.col}.degC")) %>%
  separate(year, into = c("year", "2014"))

# Plot ∆ED50 2014-2023 for all Species
# sp.degC %>%
#   filter(year == "2023") %>%
#   mutate(Species = fct_reorder(Species, ed50.degC)) %>%
#   ggplot(aes(x = Species, y = ed50.degC)) +
#   geom_point() +
#   #geom_errorbar(aes(ymin = lower.degC, ymax = upper.degC), width = 0) +
#   geom_errorbar(aes(ymin = ed50.degC - se.degC, ymax = ed50.degC + se.degC), width = 0.2) +
#   theme_classic() +
#   theme(axis.text.x = element_text(angle = 90)) +
#   labs(y = "Change in heat tolerance (°C) 2014-2023")
...
out <- full_join(slopes, nets) %>%
  mutate(`2014-2015-2023` = slope * 0.0777 * 10,
         `2014-2023` = neted50/9 * 0.0777 * 10)
...
fin <- out %>%
  select(Species, degC.dec = `2014-2023`, degC.dec.se = neted50.SE)
```

## Tolerance vs. density changes, 2014-2013
...
tol_dens_plot <- ggplot(net, aes(x = estimate, y = degC.dec)) + 
  geom_errorbar(aes(ymin = degC.dec - degC.dec.se * 0.0777, 
                    ymax = degC.dec + degC.dec.se * 0.0777), width = 0) +
  ...
  theme_classic()
ggsave(filename = "output/tol_dens_plot.png", plot = tol_dens_plot,
       width = 95, height = 90, units = "mm")
