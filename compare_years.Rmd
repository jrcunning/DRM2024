...
## Group-level instead of subject-level model -- fits faster
allg <- allf %>%
...
```{r SPECIES_diffs_by_dhw_by_year}
...
mod_1 <- glm(n ~ Species * year * Subregion, data = dens, family = "poisson") 
anova(mod_1)

res <- emmeans(mod_1, specs = c("Species", "year", "Subregion"), type = "response")

as.tibble(res) %>%
  ggplot(aes(x = year, y = rate)) +
  geom_point(aes(color = Subregion)) +
  #geom_errorbar(aes(ymin = rate - SE, ymax = rate + SE)) +
  geom_line(aes(group = Subregion, color = Subregion)) +
