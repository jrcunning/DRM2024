...
dhw.sp.res <- emmeans(dhw.sp.mod, specs = c("Species", "dhw", "year", "Week2"), type = "response",
                   at = list(dhw = seq(0, 23, 1)), rg.limit = 22000)
...
# # Find significant differences at 6 DHW
# dhw.sp.res3 <- emmeans(dhw.sp.mod, specs = c("Species", "dhw", "year", "Week2"), type = "response",
#                    at = list(dhw = 6), rg.limit = 22000)
# dhw.sp.res4 <- subset(dhw.sp.res3,
#   (year == "2014" & Week2 == "(36,38]") |
#   (year == "2015" & Week2 == "(36,38]") |
#   (year == "2022" & Week2 == "(37,39]") |
#   (year == "2023" & Week2 == "(33,35]") 
# )
# dhw.sp.res4
# rbind(pairs(dhw.sp.res4, specs = "year", by = "Species"), adjust = "fdr")
...
# Find where confidence intervals are either fully below or fully above 0.5
## (Assess which yearly differences are significant)
cnat <- as.tibble(dhw.sp.res2) %>%
  filter(Species == "CNAT") %>%
  mutate(diff0.5 = case_when(asymp.LCL > 0.5 ~ "higher",
                             asymp.UCL < 0.5 ~ "lower",
                             TRUE ~ "same")) %>%
  filter(diff0.5 %in% c("higher", "lower"))

cnat %>% 
  group_by(year, diff0.5) %>%
  summarize(min = min(dhw),
            max = max(dhw))

# Get yearly DHW diffs to 50% bleaching -- convert to °C
ok <- as.tibble(dhw.sp.res22) %>%
  group_by(Species, year) %>%
  slice(which.min(abs(prob - 0.5))) %>%
  ungroup() %>%
  dplyr::select(year, Species, dhw) %>%
  pivot_wider(names_from = year, values_from = dhw) %>%
  mutate(diffdhw2015 = `2015` - `2014`,
         diffdhw2023 = `2023` - `2014`,
         diffC2015 = diffdhw2015 * 0.077,
         diffC2023 = diffdhw2023 * 0.077)

ok %>% arrange(diffC2023)
```
