...
ggplot(sppbleachprob, aes(x = dhw.bin, y = response, color = year, group = year)) +
  geom_point() +
  geom_line() +
  geom_ribbon(aes(ymin = asymp.LCL, ymax = asymp.UCL), lwd = 0, alpha = 0.3) +
  facet_wrap(~Species) +
...
# Consider lumping Agaricia spp., Mycetophyllia spp., Oculina spp., Solenastrea spp.
all <- all %>%
  mutate(Species = case_when(Species %in% c("AFRA", "AGAR", "AHUM", "ALAM") ~ "AGAR",
                             Species %in% c("MALI", "MLAM", "MYCE") ~ "MYCE",
                             Species %in% c("OCUL", "ODIF") ~ "OCUL",
                             Species %in% c("PFUR", "PDIV", "PORI") ~ "PORI",
                             Species %in% c("SHYA", "SBOU") ~ "SOLE",
                             TRUE ~ Species)) %>%
  drop_na(Species)

# Keep only those observed at least 10 times per year
spps <- all %>%
  count(Species, year) %>%
  group_by(Species) %>%
...
#library(speedglm)
m <- glm(Bleaching2 > 1 ~ year * Species * dhw.bin, family = binomial(), data = all)

# Get fitted values across dhw [normalize to Aug13-26 window of bleaching rates]
res <- emmeans(m, specs = c("Species", "dhw.bin", "year"), rg.limit = 20000, type = "response")
