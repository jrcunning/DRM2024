...
### Species differences in bleaching in 2023 with increasing DHWs
...
# In 2014: 3.9 DHW. 2015: 8.1, In 2023: 11.2 DHW.
# 11.7 - 3.9 = 7.8 DHW increase

degC <- bleach50 %>%
  ungroup() %>%
  mutate(diffdhw = dhw - dhw[year == "2014"],
         diffthresh = diffdhw * 0.07770643,
         lower = diffdhw * 0.07527467,
         upper = diffdhw * 0.08013819)

ggplot(degC, aes(x = as.numeric(as.character(year)), y = diffthresh)) + 
  geom_point() + 
  geom_errorbar(aes(ymin = lower, ymax = upper)) +
  geom_smooth(method = "lm", se = FALSE)

mod <- lm(diffthresh ~ as.numeric(as.character(year)), data = degC)
summary(mod)
anova(mod)
# 0.0504°C/year = 0.5°C/decade
