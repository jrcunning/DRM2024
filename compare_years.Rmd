...
boot.out <- bootMer(dhw.mod, FUN = ed50s.fun, nsim = 48, seed = 123, re.form = NA,
                    parallel = "multicore", ncpus = 8)
## Get confidence intervals from Bootstrap
lower <- apply(boot.out$t, 2, function(x) as.numeric(quantile(x, probs=.08, na.rm=TRUE)))
upper <- apply(boot.out$t, 2, function(x) as.numeric(quantile(x, probs=.92, na.rm=TRUE)))
...
dhw.bleach.plot +
  geom_errorbar(data = myconf, aes(y = 0.5, xmin = lower, xmax = upper), width = 0.03)
