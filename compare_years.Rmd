...
# Bleaching variability across and within species
...
# Intra-species bleaching variability
...
# Overall bleaching susceptibility in 2023 vs. 2015 vs. 2014
...
# Get point estimates for ED50s (DHW's that cause 50% bleaching)
sp.bleach50 <- ed50s.fun(dhw.sp.mod)
sp.bleach50

# Bootstrap the calculation of ED50
## re.form = NA: does not account for random effects (conf interval), re.form = NULL: does (prediction interval)
## https://stackoverflow.com/questions/67098467/on-the-predict-mermod-function-arguments
sp.boot.out <- bootMer(dhw.sp.mod, FUN = ed50s.fun, nsim = 200, seed = 123, re.form = NULL,
                    parallel = "multicore", ncpus = 20)
## Get confidence intervals from Bootstrap
...
# Get changes relative to 2014, convert to degC
sp.degC <- sp.ed50conf %>%
...
sp.degC %>%
  ...
  geom_errorbar(aes(ymin = ed50.degC - se.degC, ymax = ed50.degC + se.degC))
...
bleach_dens <- right_join(as.tibble(densres), sp.ed50conf, by = c("Species", "year"))

# Plot 
bleach_dens %>%
  ggplot(aes(x = rate, y = ed50.degC)) +
...
# net <- as.tibble(densres) %>%
#   group_by(Species) %>%
#   summarize(netdens = rate[year == "2023"] - rate[year == "2014"],
#             logratiodens = log(rate[year == "2023"]/rate[year == "2014"])) %>%
#   left_join(filter(sp.degC, year == "2023"))

net <- sp.degC %>%
  filter(year == "2023") %>%
  select(Species, ed50.degC, se.degC, lower.degC, upper.degC) %>%
  left_join(select(as.tibble(diffs), Species, estimate, SE)) %>%
  drop_na()

ggplot(net, aes(x = estimate, y = ed50.degC)) + 
...
mod <- lm(ed50.degC ~ estimate, data = net)
anova(mod)

# Passing Bablok regression
PBreg <- mcr::mcreg(net$estimate+2, net$ed50.degC,
               method.reg = "PaBa",  method.ci = "bootstrap", nsamples = 9999)
summary(PBreg)
PBfit <- mcr::calcResponse(PBreg, x.levels = seq(0.645, 3.178, 0.1), 
                      alpha = 0.1) %>% as_tibble()
PBfit <- PBfit %>%
  mutate(estimate = X - 2,
         ed50.degC = Y)
