...
# In 2014: 3.9 DHW. 2015: 7.5, In 2023: 11.5 DHW.
...
### Better way of calculating 50% bleaching level and confidence interval? bootstrapping?
# Can we bootstrap the model to run many permutations and calculate ED50 w/confint?
ed50s.fun <- function(dhw.mod) {
  ...
  bleach50 <- as.tibble(dhw.res2) %>%
    ...
    slice(which.min(abs(prob - 0.5))) %>%
    pull(dhw)
  setNames(bleach50, c("2014", "2015", "2023"))
  return(bleach50)
}
...
boot.out <- bootMer(dhw.mod, FUN = ed50s.fun, nsim = 10, seed = 123)

myconf <- confint(boot.out)
```
