...
allg_shift <- allg %>% 
  dplyr::select(year, Week2, Subregion, Site, dhw0 = dhw, Species, BL, NB) %>%
...
allg_shift2 <- allg_shift %>%
  tidyr::drop_na() %>%
  dplyr::select(BL, NB, dhw0, dhw, year, Week2, Species, Subregion, pixel, date) 


allg_shift2 %>%
  ggplot(aes(x = dhw, y = BL / (BL + NB))) +
  geom_point() +
  facet_wrap(~year)


allg_shift2 %>%
  ggplot(aes(x = dhw0, y = dhw)) +
...
# Get bootstrap confidence intervals for ED50s and ∆ED50s across years
# set.seed(1)
# boot.shift2 <- bootMer(dhw.mod24.shift, FUN = ed50s.fun_fast, nsim = 10, re.form = NULL,
#                     seed = 123, parallel = "multicore", ncpus = 20)
