...
figs7 <- comp_std %>% left_join(ress) %>%
  ...
  geom_col(width = 0.85) +
  facet_wrap(~ Species + hel, ncol = 3, labeller = labeller(Species = abbrev_names)) +
  scale_y_continuous(labels = scales::percent_format(accuracy = 1)) +
  scale_fill_manual(values = c("#b3cde3", "#ccebc5", "#fbb4ae", "#decbe4")) +
  labs(x = NULL, y = "Depth-standardized symbiont composition", fill = "Genus") +
  theme_classic(base_size = 9) +
  theme(legend.text = element_text(face = "italic"),
        strip.text = element_text(margin = margin(t = 5, b = 0, l = 0, r = 0), face = "italic"),
        strip.background = element_blank())
figs7

ggsave(filename = "figures/FigS7.png", width = 120, height = 100, units = "mm")
...
mod1_rob <- MASS::rlm(
  ...
  psi = MASS::psi.huber,  # default; can also try psi.bisquare
  maxit = 100
)
summary(mod1_rob)
mod1_rob_ftest <- sfsmisc::f.robftest(mod1_rob, var = "med")
mod1_rob_pval <- mod1_rob_ftest$p.value

# adjusted response on the response scale
sym_shifts <- sym_shifts %>%
  mutate(
    Adj_degC_med = degC_w - coef(mod1_rob)[["estimate"]] * estimate
  )
...
regression_line_med <- tibble(
  med = med_seq,
  fit = coef(mod1_rob)[["(Intercept)"]] + coef(mod1_rob)[["med"]] * med_seq
)
...
mod2_rob <- rlm(degC_w ~ estimate + ngen_centered, data = sym_shifts, weights = 1/degC_w.se^2)
summary(mod2_rob)
mod2_rob_ftest <- sfsmisc::f.robftest(mod2_rob, var = "ngen_centered")
mod2_rob_pval <- mod2_rob_ftest$p.value
...
```{r}
# FINAL FIGURE for proxy validation
cowplot::plot_grid(p1, p2, p3, nrow = 3, align = "v", labels = "AUTO")

ggsave(filename = "figures/FigS8.png", width = 90, height = 200, units = "mm")
```
