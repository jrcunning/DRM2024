...
# APPROACH 1
dat_depth <- allf %>%
  filter(!is.na(Depth), !is.na(Species), !is.na(year), !is.na(Subregion)) %>%
  mutate(
    year = factor(year),
...
mod_depth_all <- lmer(
  Depth ~ year * Species + (1 | Subregion) + (1 | Species:Subregion),
  data = dat_depth
)
summary(mod_depth_all)

emm <- emmeans(mod_depth_all, ~ year | Species)

contrast_2014_2023 <- contrast(
  emm,
  method = list("2014 - 2023" = c(1, 0, -1, 0)),  # assumes 4 levels: 2014, 2015, 2023, 2024
  adjust = "none"
)
pairs_2014_2023 <- as.data.frame(contrast_2014_2023)
pairs_2014_2023 <- pairs_2014_2023 %>%
  dplyr::mutate(
    p.adj = p.adjust(p.value, method = "bonferroni")  # or "fdr"
  ) %>%
  dplyr::arrange(p.adj)
pairs_2014_2023

depthshift <- pairs_2014_2023 %>%
  mutate(depthshift = estimate)


# # APPROACH 2
# 
# mod_depth <- lmer(
#   siteDepth ~ year * Species + (1 | Subregion),
#   data = allg
# )
# 
# emm <- emmeans(mod_depth, ~ year | Species)
# 
# contrast_2014_2023 <- contrast(
#   emm,
#   method = list("2014 - 2023" = c(1, 0, -1, 0)),  # assumes 4 levels: 2014, 2015, 2023, 2024
...
# )
# 
# rbind(contrast_2014_2023)


# APPROACH 3
# make sure count exists
allg_c <- allg %>%
  mutate(count = BL + NB)

# define surveyed site-years (one row per surveyed site in a given year)
survey_keys <- allg_c %>%
  distinct(year, Subregion, Site, siteDepth)

# complete to explicit zeros for all species at each surveyed site-year
allg_full <- allg_c %>%
  dplyr::select(year, Subregion, Site, siteDepth, Species, count) %>%
  complete(
    nesting(year, Subregion, Site, siteDepth),
    Species,
    fill = list(count = 0)
  ) %>%
  mutate(
...
dat_1423 <- dat_1423 %>%
  mutate(
    depth_sc = as.numeric(scale(siteDepth))
  )

depth_center <- mean(dat_1423$siteDepth, na.rm = TRUE)
depth_scale  <- sd(dat_1423$siteDepth, na.rm = TRUE)

fit_depth_model <- function(df_sp) {
  df_sp <- droplevels(df_sp)

  # skip species with too little information
  info <- df_sp %>%
    group_by(year) %>%
    summarise(
      n_sites = n(),
  ...
  if (nrow(info) < 2 || any(info$n_nonzero < 3)) {
    return(list(
      model = NULL,
      family = NA_character_,
      ok = FALSE,
      reason = "too few nonzero observations in one or both years"
    ))
  }

  # try NB first
  mod_nb <- try(
    MASS::glm.nb(count ~ year * depth_sc + Subregion, data = df_sp),
    silent = TRUE
  )
  ...
  # fallback to Poisson
  mod_pois <- try(
    glm(count ~ year * depth_sc + Subregion,
        data = df_sp,
        family = poisson()),
    silent = TRUE
  )

  if (!inherits(mod_pois, "try-error")) {
    return(list(
      model = mod_pois,
      family = "poisson",
      ok = TRUE,
      reason = "NB failed; used Poisson fallback"
    ))
  }
  ...
  list(
    ...
    family = NA_character_,
    ok = FALSE,
    reason = "both NB and Poisson failed"
  )
}

mods_by_species <- dat_1423 %>%
  group_split(Species) %>%
  set_names(levels(dat_1423$Species)) %>%
  map(fit_depth_model)

extract_depth_metrics <- function(fit_obj, sp_name) {
  if (!fit_obj$ok || is.null(fit_obj$model)) {
    return(tibble(
      Species = sp_name,
      fit_ok = FALSE,
      family = fit_obj$family,
      reason = fit_obj$reason,
      beta_year = NA_real_,
      se_year = NA_real_,
      p_year = NA_real_,
      beta_depthshift = NA_real_,
      se_depthshift = NA_real_,
      p_depthshift = NA_real_
    ))
  }

  tt <- broom::tidy(fit_obj$model)

  yr_row <- tt %>% filter(term == "year2023")
  int_row <- tt %>% filter(term == "year2023:depth_sc")
  ...
  tibble(
    ...
    family = fit_obj$family,
    reason = fit_obj$reason,
    beta_year = ifelse(nrow(yr_row) == 1, yr_row$estimate, NA_real_),
    se_year   = ifelse(nrow(yr_row) == 1, yr_row$std.error, NA_real_),
    p_year    = ifelse(nrow(yr_row) == 1, yr_row$p.value, NA_real_),
    beta_depthshift = ifelse(nrow(int_row) == 1, int_row$estimate, NA_real_),
    se_depthshift   = ifelse(nrow(int_row) == 1, int_row$std.error, NA_real_),
    p_depthshift    = ifelse(nrow(int_row) == 1, int_row$p.value, NA_real_)
  )
}

depth_metrics <- imap_dfr(mods_by_species, extract_depth_metrics)
depth_metrics

depth_metrics <- depth_metrics %>%
  mutate(
    p_year_adj       = p.adjust(p_year, method = "fdr"),
    p_depthshift_adj = p.adjust(p_depthshift, method = "fdr")
  ) %>%
  arrange(p_depthshift_adj)

depth_metrics

hi <- depth_metrics %>% right_join(net_clean, by = "Species") %>%
  mutate(beta_depthshift = -beta_depthshift)
ggplot(hi, aes(x = estimate, y = beta_depthshift)) + geom_point() + geom_text(aes(label = Species)) + ggpubr::stat_cor()
ggplot(hi, aes(x = estimate, y = depthshift)) + geom_point() + geom_text(aes(label = Species)) + ggpubr::stat_cor()
ggplot(hi, aes(x = depthshift, y = beta_depthshift)) + geom_point() + geom_text(aes(label = Species)) + ggpubr::stat_cor()

mod_mech <- lm(
    degC_w ~ estimate + ngen + sizechange + sus_group + beta_depthshift,
    data = hi,
...
mod_mech2 <- lm(
    degC_w ~ estimate + ngen + sizechange + sus_group + depthshift,
    data = hi,
    weights = 1/degC_w.se^2
)
summary(mod_mech2)

cor(hi$depthshift, hi$beta_depthshift)
```
...
summary(dhw.mod24.depth)
d1 <- emmeans(dhw.mod24, specs = c("year", "dhw"), type = "response", at = list(dhw = seq(0,20,1)))
d2 <- emmeans(dhw.mod24.depth, specs = c("year", "dhw"), type = "response", at = list(dhw = seq(0,20,1)))
ggplot(data.frame(d1), aes(x = dhw, y = prob, group = factor(year), color = factor(year))) + geom_line()
ggplot(data.frame(d2), aes(x = dhw, y = prob, group = factor(year), color = factor(year))) + geom_line()
...
# 0.90°C [95% C.I. = 0.78-1.03°C]
finaldegC24 <- degC %>% 
  filter(year == 2024) %>%
  dplyr::select(degC, degC.lower, degC.upper)
# 1.47°C [95% C.I. = 1.32-1.63°C]

# Annotate bleaching response curves plot // Fig3a
fig3a <- dhw.bleach.plot +
  annotate("text", x = 11.1, y = 0.04, hjust = 0, size = 2, 
           label = paste0("+", signif(ed50conf$ed50[[6]], 2), " DHW\n", 
                          "= +", signif(finaldegC23$degC, 2), " °C")) +
  annotate("text", x = 16.5, y = 0.04, hjust = 0, size = 2, fontface = "italic",
...
dhw.sp.mod.24.depth <- glmer(
  ...
  family = "binomial", data = allg, verbose = FALSE,
  nAGQ=0, control=glmerControl(optimizer = "nloptwrap"))
anova(dhw.sp.mod.24, dhw.sp.mod.24.depth)
summary(dhw.sp.mod.24.depth)
d1 <- emmeans(dhw.sp.mod.24, specs = c("year", "dhw", "Species"), type = "response", 
              at = list(dhw = seq(0,20,1)), rg.limit = 200000)
d2 <- emmeans(dhw.sp.mod.24.depth, specs = c("year", "dhw", "Species"), type = "response", 
              at = list(dhw = seq(0,20,1)), rg.limit = 200000)
ggplot(data.frame(d1), aes(x = dhw, y = prob, group = factor(year), color = factor(year))) + 
  geom_line() + facet_wrap(~Species)
ggplot(data.frame(d2), aes(x = dhw, y = prob, group = factor(year), color = factor(year))) + 
  geom_line() + facet_wrap(~Species)
...
dhw.sp.res.24 <- emmeans(dhw.sp.mod.24, specs = c("Species", "dhw", "year", "Week2"), type = "response",
...
dhw.sp.res.24.depth <- emmeans(dhw.sp.mod.24.depth, specs = c("Species", "dhw", "year", "Week2"), type = "response",
                      at = list(dhw = seq(0, maxdhw, 0.1), Week2 = c("(33,35]", "(35,37]", "(37,39]")), 
                      rg.limit = 200000, level = 0.84)
dhw.sp.res.24.2.depth <- subset(dhw.sp.res.24.depth,
  (year == "2014" & Week2 == "(35,37]" & dhw <= 7.12 & dhw > 0.48) |
  (year == "2015" & Week2 == "(37,39]" & dhw <= 8.00 & dhw > 0) |
  (year == "2023" & Week2 == "(33,35]" & dhw <= 20.4 & dhw > 1.46) |
  (year == "2024" & Week2 == "(37,39]" & dhw <= 11.5 & dhw > 0.30) 
...
# Plot
(dhw.sp.bleach.plot.24 <- ggplot(as.tibble(dhw.sp.res.24.2.depth), aes(x = dhw, y = prob, group = year)) +
...
sp.ed50s.fun.24 <- function(dhw.sp.mod) {
  dhw.sp.res <- emmeans(dhw.sp.mod, specs = c("Species", "dhw", "year", "Week2"), type = "response",
                   at = list(dhw = seq(0, 21, 0.05), Week2 = c("(33,35]", "(35,37]", "(37,39]")), 
                   rg.limit = 200000, level = 0.84)
  dhw.sp.res22 <- subset(dhw.sp.res,
    (year == "2014" & Week2 == "(35,37]") |
    (year == "2015" & Week2 == "(37,39]") |
...
# Get point estimates for ED50s and ∆ED50s
sp.bleach50.24 <- sp.ed50s.fun.24(dhw.sp.mod.24)
...
# sp.boot.out.24 <- bootMer(dhw.sp.mod.24, FUN = sp.ed50s.fun.24, nsim = 100, seed = 123, re.form = NULL,
...
# 
# ggplot(sp.degC, aes(x = Species, y = degC)) +
#   geom_point() +
#   geom_errorbar(aes(ymin = degC - degC.se, ymax = degC + degC.se))
# 
# 
...
# 
# sp.degC %>% arrange(-degC) %>% print(n = nrow(.))
```
...
sp.degC.w[which(sp.degC.w$Species == "PDIV"), "degC_w.se"] <- 0.0495
...
net <- sp.degC.w %>%
  ...
  drop_na(estimate)
# net <- net %>%
#   filter(!Species %in% c("PFUR", "PDIV", "AAGA"))
...
label_decomp <- sprintf(
  "Modeled drivers account for %.0f%% of total |Δ°C|\n(Symbionts %.0f%%, Density %.0f%%)",
  pct_modeled,
  pct_symb,
  pct_density
)
...
# cowplot::plot_grid(dens_sym_viz, p1, p2, nrow = 1, rel_widths = c(1,1,1), labels = "AUTO")
# ggsave(filename = "figures/Fig5alt2.png", width = 300, height = 95, units = "mm")
...
```{r}
# ----------------------------------------------
# Test whether changes in colony size or SCTLD susceptibility 
...
# add depth shift?
net_clean <- net_clean %>%
  left_join(depthshift %>% dplyr::select(Species, depthshift), by = "Species")
...
mod_full <- lm(
  degC_w ~ estimate + ngen + logdens2014 + sizechange + sus_group + depthshift,
  data = dat,
  weights = w
)
summary(mod_full)
...
visreg(mod_full, "depthshift")
ggplot(dat, aes(x = estimate, y = depthshift)) +
  geom_point() +
  geom_text(aes(label = Species))
cor(dat$estimate, dat$depthshift)
...
modB <- lm(
  degC_w ~ depthshift + ngen + logdens2014 + sizechange + sus_group,
  data = dat,
  weights = w
)
modC <- lm(
  degC_w ~ estimate + depthshift + ngen + logdens2014 + sizechange + sus_group,
  data = dat,
  weights = w
)
anova(modC, modB)
...
mod_uni_depth <- lm(degC_w ~ depthshift,   data = dat, weights = w)
...
pred_depth  <- make_cont_pred(mod_uni_depth, dat, "depthshift")
...
ann_depth <- make_num_ann(mod_uni_depth, "depthshift")
...
p_uni_depth <- ggplot(dat, aes(x = depthshift, y = degC_w)) +
  geom_point(alpha = 0.8) +
  geom_ribbon(
    data = pred_depth,
    aes(x = depthshift, ymin = lwr, ymax = upr),
    inherit.aes = FALSE,
    alpha = 0.2
  ) +
  geom_line(
    data = pred_depth,
    aes(x = depthshift, y = fit),
    inherit.aes = FALSE,
    linewidth = 0.9, color = "blue"
  ) +
  geom_text(
  ...
  coord_cartesian(ylim = ylims) +
  labs(x = "Change in depth", y = "Change in\nbleaching threshold (°C)") +
  theme_bw()
