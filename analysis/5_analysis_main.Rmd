...
dhw_daily <- dhw_daily %>%
  group_by(year) %>%
  arrange(date) %>%
  mutate(mean_dhw_smooth = zoo::rollmean(mean_dhw, k = 21, fill = NA)) %>%
  ungroup()

ggplot(dhw_daily, aes(x = date, y = mean_dhw)) +
  geom_line() +
...
peak_dates <- dhw_daily %>%
  group_by(year) %>%
  slice_max(mean_dhw_smooth, n = 1, with_ties = FALSE) %>%
  ungroup()
...
ggplot(allg, 
       aes(x = dhw, y = BL/(BL+NB))) +
  geom_point(aes(size = BL+NB, color = Subregion), alpha = 0.25) +
  facet_wrap(~year) +
  geom_smooth(method = "loess")

ggplot(allg, 
       aes(x = cummaxdhw, y = BL/(BL+NB))) +
  geom_point(aes(size = BL+NB, color = Subregion), alpha = 0.25) +
  facet_wrap(~year) +
  geom_smooth(method = "loess")
...
ggplot(allg, aes(x = dhw, y = BL/(BL+NB))) +
  #geom_point(aes(size = BL+NB), alpha = 0.25) +
  #facet_wrap(~year) +
  geom_smooth(aes(group = year, color = year), method = "loess")

ggplot(allg, aes(x = Week2, y = BL/(BL+NB))) +
  geom_jitter() +
  geom_boxplot() +
...
ed50s.fun <- function(dhw.mod) {
  dhw.res <- emmeans(dhw.mod, specs = c("dhw", "year", "Week2"), type = "response",
                     at = list(dhw = seq(0, 23, 0.1), Week2 = c("(36,38]", "(38,40]")), 
                     rg.limit = 50000, level = 0.84)
  dhw.res2 <- subset(dhw.res,
                     (year == "2014" & Week2 == "(36,38]") |
                     (year == "2015" & Week2 == "(36,38]") |
                     (year == "2023" & Week2 == "(36,38]") |
  ...
  diff2024 <- bleach50[4]-bleach50[1]
  res <- c(bleach50, diff2015, diff2023, diff2024)
  res <- setNames(res, c("2014", "2015", "2023", "2024", "2015-2014", "2023-2014", "2024-2014"))
  return(res)
}
...
boot.out <- bootMer(dhw.mod, FUN = ed50s.fun, nsim = 100, seed = 123, re.form = NULL,
...
ggplot(nd, aes(x = dhw, y = prob)) +
  geom_line(aes(color = Week2)) +
  facet_wrap(~year) +
...
dhw.res2 <- subset(dhw.res,
  (year == "2014" & Week2 == "(36,38]" & dhw <= 8.11 & dhw > 0.48) |
  (year == "2015" & Week2 == "(38,40]" & dhw <= 8.90 & dhw > 0) |
  (year == "2023" & Week2 == "(36,38]" & dhw <= 21.5 & dhw > 1.06) |
