...
peak_dates <- peak_dates %>%
  select(year, peak_date_pred, Week2 = Week2_pred, dhw_max, peak_date_obs, n_rise)
peak_dates

# --- build fitted line on the rising-limb subset for plotting ---
rise_fit_df <- dhw_daily %>%
  inner_join(peak_dates %>% select(year, dhw_max, peak_date_obs, peak_date_pred), by = "year") %>%
  group_by(year) %>%
  group_modify(~{
    d <- .x %>% arrange(date)
    d_rise <- d %>%
...
ggsave(filename = "figures/Fig4.png", width = 100, height = 90, units = "mm")
...
fig3 <- cowplot::plot_grid(fig4, spfig, nrow = 1, rel_widths = c(0.6, 0.4), align = "hv", labels = "AUTO")
fig4alt
...
ols_model_adjusted <- lm(degC ~ estimate + ngen_centered, data = net_clean, weights = 1/degC.se^2)
summary(ols_model_adjusted)
...
(decomp <- ggplot(out, aes(x = Species, y = value, fill = name)) +
  ...
  geom_point(aes(y = degC, shape = "Observed change (±SE)"), size = 2) +
