...
# Plot
ggplot(dhw_daily, aes(x = date_x, color = factor(year))) +
  geom_line(aes(y = mean_dhw), linewidth = 0.25) +
  geom_line(aes(y = mean_dhw_smooth)) +
  scale_x_date(date_breaks = "1 month", date_labels = "%b") +
  geom_vline(data = peak_dates, aes(xintercept = date_x, color = factor(year)), linetype = 2)
...
# Filter out Marquesas Subregion -- it was not surveyed in 2014 or 2015
# Filter out Tortugas Bank bc only surveyed in 2024
allf <- allf %>% filter(!Subregion %in% c("Marquesas", "Tortugas--Tortugas Bank"))

## Group data at species+site-level to fit group-level models instead of subject-level models -- fits faster
allg <- allf %>%
  group_by(year, Date, Week2, Subregion, Site, dhw, cummaxdhw, Species) %>%
  summarize(BL = sum(Bleaching2 > 1),
            NB = sum(Bleaching2 <= 1)) %>%
  ungroup()
...
ggplot(allg, aes(x = dhw, y = BL/(BL+NB))) +
  geom_smooth(aes(group = year, color = year), method = "loess")

ggplot(allg, aes(x = cummaxdhw, y = BL/(BL+NB))) +
  geom_point(aes(size = BL+NB), alpha = 0.1) +
  facet_wrap(~year) +
  geom_smooth(method = "loess")

ggplot(allg, aes(x = dhw, y = cummaxdhw)) +
...
ed50s.fun_fast <- function(mod) {
  combos <- peak_dates
  b <- fixef(mod)
  # global DHW slope (same for all year/week in your model)
  b_dhw <- unname(b["dhw"])
  ...
  # compute eta at dhw = 0 for each combo under your parameterization
  eta0 <- numeric(nrow(combos))
  for (i in seq_len(nrow(combos))) {
    yr <- combos$year[i]
    wk <- combos$Week2[i]
    eta <- getb("(Intercept)")
    # year main effects: only present for non-reference years (reference appears to be 2014)
    if (yr != "2014") eta <- eta + getb(paste0("year", yr))
    # interaction term uses: yearXXXX:Week2(...)
    # (note: in your fixef output, even 2014 has interaction columns)
    eta <- eta + getb(paste0("year", yr, ":Week2", wk))
    eta0[i] <- eta
  }
  # ED50 is where eta = 0 on logit scale => dhw = -eta0 / b_dhw
  ed50 <- if (abs(b_dhw) < 1e-12) rep(NA_real_, length(eta0)) else -eta0 / b_dhw
  res <- c(ed50, ed50[2]-ed50[1], ed50[3]-ed50[1], ed50[4]-ed50[1])
  setNames(res, c("2014","2015","2023","2024","2015-2014","2023-2014","2024-2014"))
}

# Get point estimates for ED50s and ∆ED50s
bleach50 <- ed50s.fun_fast(dhw.mod)
bleach50

# Get bootstrap confidence intervals for ED50s and ∆ED50s across years
# set.seed(1)
# boot.out <- bootMer(dhw.mod, FUN = ed50s.fun_fast, nsim = 100,
#                     seed = 123, re.form = NULL, parallel = "multicore", ncpus = 20)
...
dhw.bleach.plot <- ggplot(as.tibble(dhw.res2), aes(x = dhw, y = prob, group = year)) +
  ...
  geom_ribbon(aes(ymin = asymp.LCL, ymax = asymp.UCL), lwd = 0, alpha = 0.1) +
  geom_line(aes(color = prob), lwd = 2) +
  geom_text(data = slice(ed50conf, 1:4),
           aes(x = c(8,9.5,20.5, 13.5), y = c(0.8, 0.65, 0.97, 0.18), label = year)) +
...
```{r}
# sanity check - how week2 affects predictions
allg <- allg %>%
  mutate(yrWk = interaction(year, Week2, drop = TRUE))

dhw.mod2 <- glmer(cbind(BL, NB) ~ dhw + 0 + yrWk + (dhw + year | Species) + (year | Subregion),  
                 family = "binomial", data = allg, 
                 verbose = FALSE, nAGQ = 0, control = glmerControl(optimizer = "nloptwrap"))

nd <- allg %>%
  distinct(year, yrWk) %>%
  crossing(dhw = seq(0, 23, 0.1))

nd$prob <- predict(dhw.mod2, newdata = nd, type = "response", re.form = NA) 

nd <- nd %>%
  separate(yrWk, into = c("year", "Week2"), sep = "\\.")

# Get last x-value per line (within each year × Week2)
nd_lab <- nd %>%
  group_by(year, Week2) %>%
  filter(dhw == max(dhw)) %>%
  ungroup()
...
library(ggrepel)
ggplot(nd, aes(x = dhw, y = prob, color = Week2)) +
  geom_line() +
  facet_wrap(~year) +
  geom_text_repel(
    data = nd_lab,
...
```{r}
# CUMMAXDHW
## --- cummaxdhw sanity-check model + plot + ED50 bootstrap (robust) ---
...
## 0) IMPORTANT NOTE ON FILTER
## This filter keeps observations where dhw is NOT below the running max-to-date.
## Because dhw <= cummaxdhw by definition, this typically keeps rows where dhw == cummaxdhw
## (i.e., record-high days). If that’s what you want for this sanity check, keep it.
allg2 <- allg %>%
  filter(!(dhw < cummaxdhw))

## 1) Fit model (explicitly make sure key cols are factors)
allg2 <- allg2 %>%
  mutate(
    year = factor(year),
...
dhw.mod4 <- glmer(
  cbind(BL, NB) ~ cummaxdhw + year +
    (cummaxdhw + year | Species) +
    (year | Subregion),
  family  = binomial,
  data    = allg2,
  verbose = FALSE,
...
## 2) Stabilize dataset used by the fitted model (prevents bootMer row-mismatch errors)
mf_used <- model.frame(dhw.mod4)
used_idx <- as.integer(rownames(mf_used))
allg2_used <- allg2[used_idx, ]

dhw.mod4_used <- update(dhw.mod4, data = allg2_used)

## 3) Plot population-average curves (fixed effects only) using emmeans
## Use CI columns, NOT prob ± SE
dhw.mod4.res <- emmeans(
  dhw.mod4_used,
  specs = c("cummaxdhw", "year"),
  type  = "response",
...
plot_df <- as_tibble(dhw.mod4.res)

dhw.bleach.plot <- ggplot(plot_df, aes(x = cummaxdhw, y = prob, group = year)) +
  geom_ribbon(aes(ymin = asymp.LCL, ymax = asymp.UCL), linewidth = 0, alpha = 0.1) +
  geom_line(aes(color = prob), linewidth = 2) +
  annotate(
    "segment", x = 3.9, xend = 11.5, y = 0.05, linewidth = 0.2,
    arrow = arrow(type = "closed", length = unit(3, "mm"))
  ) +
  scale_x_continuous(limits = c(-1, 22), breaks = seq(0, 24, 4), expand = c(0, 0)) +
  scale_y_continuous(limits = c(0, 1), expand = c(0, 0), breaks = seq(0, 1, 0.1),
...
## 4) ED50 function (population-average; grid + closest to 0.5)
ed50s_fun_cummax <- function(mod, step = 0.01, rng = c(0, 23)) {
  
  mf <- model.frame(mod)
  
  # lock factor levels to fitted model
  yrs <- levels(mf$year)
  
  # valid levels for grouping vars (won't matter because re.form = NA)
  sp0 <- levels(mf$Species)[1]
  sr0 <- levels(mf$Subregion)[1]
  
  nd <- tidyr::expand_grid(
    year = factor(yrs, levels = yrs),
    cummaxdhw = seq(rng[1], rng[2], by = step)
  ) %>%
    mutate(
      Species   = factor(sp0, levels = levels(mf$Species)),
      Subregion = factor(sr0, levels = levels(mf$Subregion))
    )
  ...
  ref <- yrs[1]  # reference year = first factor level
  diffs <- ed50[yrs[-1]] - ed50[ref]
  names(diffs) <- paste0(yrs[-1], "-", ref)
  
  c(ed50, diffs)
}

## 5) Point estimates
ed50_hat4 <- ed50s_fun_cummax(dhw.mod4_used)
ed50_hat4

## 6) Bootstrap CIs (robust to prior row-omissions because we refit on allg2_used)
boot.out4 <- bootMer(
  dhw.mod4_used,
  FUN = ed50s_fun_cummax,
  nsim = 100,    # increase as needed
  seed = 123,
  parallel = "multicore",
  ncpus = 20
)
saveRDS(boot.out4, file = "data/processed/boot.out4.rds")
boot.out4 <- readRDS(file = "data/processed/boot.out4.rds")

lower <- apply(boot.out4$t, 2, function(x) as.numeric(quantile(x, 0.025, na.rm = TRUE)))
upper <- apply(boot.out4$t, 2, function(x) as.numeric(quantile(x, 0.975, na.rm = TRUE)))
se    <- apply(boot.out4$t, 2, sd, na.rm = TRUE)

ed50conf4 <- tibble(
  term  = names(ed50_hat4),
  ed50  = unname(ed50_hat4),
  se    = se,
  lower = lower,
  upper = upper
)
