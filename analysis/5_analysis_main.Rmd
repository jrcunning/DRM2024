...
sp.degC.w[which(sp.degC.w$Species == "PDIV"), "degC_w.se"] <- 0.0475 # lowest value non PDIV/PFUR
sp.degC.w[which(sp.degC.w$Species == "PFUR"), "degC_w.se"] <- 0.0475 # lowest value non PDIV/PFUR
...
# # UPDATE: CONVERT TO DEGC INSIDE BOOTSTRAP? BUT CURR DOES NOT ACCOUNT FOR ERROR IN MEAN.SLOPE
# sp.boot.1423 <- sp.boot.out.24$t[,grepl("2023-2014", colnames(sp.boot.out.24$t))]
# boot_degC <- sweep(sp.boot.1423, 2, sp_mean_slope$mean_slope, "/")
# degC_se <- apply(boot_degC, 2, sd, na.rm = TRUE)
# 
# sp.degC.w2 <- sp.ed50s %>%
#   left_join(sp_mean_slope, by = "Species") %>%
#   mutate(degC_w = ed50 / mean_slope,
#          degC_w.se = degC_1423_se)
# 
# sp.degC.w2 %>% arrange(degC_w.se)
# sp.degC.w2[which(sp.degC.w2$Species == "PDIV"), "degC_w.se"] <- 0.0603 # lowest value non PDIV/PFUR
# sp.degC.w2[which(sp.degC.w2$Species == "PFUR"), "degC_w.se"] <- 0.0603 # lowest value non PDIV/PFUR
```
