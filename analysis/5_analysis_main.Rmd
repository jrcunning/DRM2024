...
dhw2014 <- read_csv("data/processed/2014_dhw_processed.csv")
dhw2015 <- read_csv("data/processed/2015_dhw_processed.csv")
dhw2023 <- read_csv("data/processed/2023_dhw_processed.csv")
dhw2024 <- read_csv("data/processed/2024_dhw_processed.csv")
...
eds <- imap_dfr(
  ...
  mutate(
    ...
    # censoring indicator: did we reach 90% bleaching (ED10_NB) by 23 DHW?
    reach = ED10_raw <= 23,

    # keep your truncated ED10 for plotting if you want
    Estimate_ED10_trunc = pmin(ED10_raw, 23),
    ...
    # if not reached, Range is right-censored at (23 - ED90)
    Range_cens = if_else(reach, ED10_raw - Estimate_ED90, 23 - Estimate_ED90),

    cens_type = if_else(reach, "none", "right")
  )

# Uncertainty on the ED90-ED10 difference is unreliable and incomparable across species since many did not actually reach 90% bleaching within the observed DHW range and therefore this end of the DRC is unconstrained. So we will compute a bounded estimate within the observed DHW range (truncate at 23).
...
figs3 <- ggplot(eds, aes(x = ngen, y = Diff_trunc)) +
  ...
  geom_text(data = anno, x = 0.9, y = 23:24, aes(label = label), color = c("blue", "black"),
            hjust = 0, size = 2.5) +
...
ggsave(filename = "figures/FigS3.png", plot = fig3, width = 100, height = 100, units = "mm")
```
...
fig3a <- dhw.bleach.plot +
  ...
  annotate("text", x = 18.4, y = 0.04, hjust = 0, size = 2, fontface = "italic",
           label = paste0("+", signif(ed50conf$ed50[[7]], 2), " DHW\n", 
...
dhw.sp.res.24 <- emmeans(dhw.sp.mod.24, specs = c("Species", "dhw", "year", "Week2"), type = "response",
                      at = list(dhw = seq(0, 23, 0.1), Week2 = c("(33,35]", "(35,37]", "(37,39]")), 
                      rg.limit = 200000, level = 0.84)
dhw.sp.res.24.2 <- subset(dhw.sp.res.24,
  (year == "2014" & Week2 == "(35,37]" & dhw <= 8.11 & dhw > 0.48) |
  (year == "2015" & Week2 == "(37,39]" & dhw <= 8.90 & dhw > 0) |
  (year == "2023" & Week2 == "(33,35]" & dhw <= 21.5 & dhw > 1.06) |
  (year == "2024" & Week2 == "(37,39]" & dhw <= 11.7 & dhw > 0.16) 
)
...
fig3 <- cowplot::plot_grid(fig3a, spfig, nrow = 1, rel_widths = c(0.6, 0.4), align = "hv", labels = "AUTO")
ggsave(filename = "figures/Fig3n.png", width = 200, height = 100, units = "mm")
...
sp.ed50s.fun.24 <- function(dhw.sp.mod) {
  dhw.sp.res <- emmeans(dhw.sp.mod, specs = c("Species", "dhw", "year", "Week2"), type = "response",
                   at = list(dhw = seq(0, 23, 0.05), Week2 = c("(33,35]", "(35,37]", "(37,39]")), 
                   rg.limit = 200000, level = 0.84)
...
# sp.boot.out.24 <- bootMer(dhw.sp.mod.24, FUN = sp.ed50s.fun.24, nsim = 500, seed = 123, re.form = NULL,
...
sp.degC.w[which(sp.degC.w$Species == "PDIV"), "degC_w.se"] <- 0.068
...
densdiffs <- read_csv("data/processed/densdiffs_smoth.csv")
...
ggsave(filename = "figures/Fig4r.png", width = 300, height = 95, units = "mm")
...
contrast(factorres, method = "consec")
## Same results --> use continuous
```

# decomp with explicit baseline intercept + residual
```{r}
## ============================================================
## Decomposition with explicit Baseline (intercept) + Residual
## ============================================================

# Fit (unchanged)
ols_model_adjusted <- lm(
  degC_w ~ estimate + ngen_centered,
  data    = net_clean,
  weights = 1 / degC_w.se^2
)

# Extract coefficients
b <- coef(ols_model_adjusted)
b0 <- unname(b["(Intercept)"])
b1 <- unname(b["estimate"])
b2 <- unname(b["ngen_centered"])

# Build clean component table (no implicit intercept-in-residual mixing)
net_decomp <- net_clean %>%
  mutate(
    Baseline_Contribution = b0,
    Estimate_Contribution = b1 * estimate,
    Ngen_Contribution     = b2 * ngen_centered,
    Residual_Contribution = degC_w - (Baseline_Contribution +
                                     Estimate_Contribution +
                                     Ngen_Contribution)
  ) %>%
  mutate(
    Check_Sum = Baseline_Contribution + Estimate_Contribution +
                Ngen_Contribution + Residual_Contribution
  )

stopifnot(isTRUE(all.equal(net_decomp$Check_Sum, net_decomp$degC_w)))

# Long format for stacked bars
out_decomp <- net_decomp %>%
  dplyr::select(
    Species, degC_w, degC_w.se,
    Baseline_Contribution, Estimate_Contribution,
    Ngen_Contribution, Residual_Contribution
  ) %>%
  pivot_longer(
    cols = c(Baseline_Contribution, Estimate_Contribution,
             Ngen_Contribution, Residual_Contribution),
    names_to = "name",
    values_to = "value"
  ) %>%
  ...
  mutate(
    name = factor(
      name,
      levels = c("Baseline_Contribution",
...
## ============================================================
## Optional: magnitude budget (% of total |Δ°C| across species)
##   - This is NOT R², just an absolute-contribution summary
## ============================================================

pct_tab <- out_decomp %>%
  group_by(name) %>%
  summarize(tot_abs = sum(abs(value)), .groups = "drop") %>%
  mutate(pct = 100 * tot_abs / sum(tot_abs))

pct_symb    <- pct_tab$pct[pct_tab$name == "Ngen_Contribution"]
pct_density <- pct_tab$pct[pct_tab$name == "Estimate_Contribution"]
pct_base    <- pct_tab$pct[pct_tab$name == "Baseline_Contribution"]
pct_resid   <- pct_tab$pct[pct_tab$name == "Residual_Contribution"]

pct_modeled <- pct_symb + pct_density  # modeled drivers only (excluding baseline)

label_decomp <- sprintf(
  "Modeled drivers account for %.0f%% of total |Δ°C|\n(Symbionts %.0f%%, Density %.0f%%)\nBaseline %.0f%%, Residual %.0f%%",
  pct_modeled, pct_symb, pct_density, pct_base, pct_resid
)

## ============================================================
## Decomposition plot (stacked components + observed points/SE)
## ============================================================

decomp2 <- ggplot(out_decomp, aes(x = Species, y = value, fill = name)) +
  geom_col(position = "stack", alpha = 1) +
  geom_point(aes(y = degC_w, shape = "Observed change (±SE)"), size = 2) +
  geom_errorbar(aes(ymin = degC_w - degC_w.se, ymax = degC_w + degC_w.se), width = 0) +
  scale_x_discrete(labels = abbrev_names[as.character(out_decomp$Species)]) +
  scale_fill_manual(
    name   = "Decomposition",
    breaks = c("Estimate_Contribution", "Ngen_Contribution",
               "Baseline_Contribution", "Residual_Contribution"),
    labels = c("∆ pop. density", "# sym. genera",
               "baseline (intercept)", "residual (unexplained)"),
    values = c(
      Estimate_Contribution = "#67a9cf",
      Ngen_Contribution     = "#ef8a62",
      Baseline_Contribution = "#bdbdbd",
      Residual_Contribution = "#7f7f7f"
    )
  ) +
  scale_shape_manual(name = "", values = 16) +
  annotate(
    "text",
    x = Inf, y = -Inf, hjust = 1, vjust = -0.4,
    label = label_decomp, size = 2.47
  ) +
  theme_classic() +
  theme(
...
ggsave(filename = "figures/FigS7.png", plot = sizeplot,
...
ggsave("figures/FigS8.png", plot = sizefreq, width = 220, height = 220, units = "mm")
```
...
sym_shifts <- read_csv("data/processed/symbiont_shifts.csv") %>%
  left_join(net %>% select(Species, degC, degC.se, ngen, estimate))
...
# ============================================================
# Fit the model, with intercept zero at density change estimate = 0 and med(Hel dist) = 0
#  Weight observations using the standard error of the degC response
# mod1 <- lm(degC ~ 0 + estimate + med, data = sym_shifts, weights = 1/degC.se^2)
# summary(mod1)
# mod1_pval <- broom::tidy(mod1)[2,5]
mod1 <- lm(degC ~ estimate + med, data = sym_shifts, weights = 1/degC.se^2)
summary(mod1)
mod1_pval <- broom::tidy(mod1)[3,5]

# Create partial residual effect plots showing density changes and symbiont diversity effects while controlling for the other predictor
#VVV another way. equivalent, but more explicit because ngen_centered = 0 and estimate = 0 are specified in predict
sym_shifts$Adj_degC_med <- predict(mod1, newdata = data.frame(estimate = 0, med = sym_shifts$med)) + residuals(mod1)
### We're adding the total residuals for both effect plots. is this correct? or do we need get partial residuals?


# Generate regression lines data using correct centering
med_seq <- seq(0, max(sym_shifts$med), length.out = 100)

pred_med <- predict(
  mod1,
  newdata = data.frame(estimate = 0, med = med_seq),
  se.fit = TRUE
)

regression_line_med <- tibble(
  med = med_seq,
  fit = as.numeric(pred_med$fit),
  se  = as.numeric(pred_med$se.fit)
) %>%
  mutate(lo = fit - 1.96 * se,
         hi = fit + 1.96 * se)

# regression_line_med <- data.frame(
#   med = med_seq,
#   predicted_degC = coef(mod1)["(Intercept)"] + coef(mod1)["med"] * med_seq)
...
p2 <- ggplot(sym_shifts, aes(x = med, y = Adj_degC_med)) +
  ...
  geom_ribbon(data = regression_line_med, aes(ymin = lo, ymax = hi, y = fit),
              alpha = 0.1) +
  geom_point(size = 3, alpha = 0.7, color = "#7F7F7F", stroke = 0) +
  #geom_errorbar(aes(ymin = Adj_degC_med - degC.se, ymax = Adj_degC_med + degC.se), 
  #              width = 0, alpha = 0.7, lwd = 0.3) +
  annotate(geom = "text", x = -Inf, y = Inf, label = paste("p = ", round(mod1_pval, 5)), 
           hjust = -0.1, vjust = 1.1) +
  ggrepel::geom_text_repel(aes(label = abbrev_names[as.character(Species)]),
                           fontface = "italic", size = 3, seed = 8) +
  labs(x = "Change in symbiont genus composition\n(2001-03 to 2019-20; Hellinger distance)", 
       y = "Model-adjusted change in bleaching threshold (°C)") +
  scale_size_continuous(range = c(2, 5)) +
  theme_classic()
...
# Fit the model, with intercept zero at estimate = 0 and ngen = 1
#  Weight observations using the standard error of the degC response
mod2 <- lm(degC ~ 0 + estimate + ngen_centered, data = sym_shifts, weights = 1/degC.se^2)
summary(mod2)
mod2_pval <- broom::tidy(mod2)[2,5]
mod2 <- lm(degC ~ estimate + ngen_centered, data = sym_shifts, weights = 1/degC.se^2)
summary(mod2)
...
# Generate regression lines data using correct centering
ngen_seq <- seq(1, max(sym_shifts$ngen), length.out = 100)
...
#   ngen = ngen_seq,
#   predicted_degC = coef(mod2)["(Intercept)"] + coef(mod2)["ngen_centered"] * (ngen_seq - 1)
# )
...
ggsave(filename = "figures/FigS6.png", width = 300, height = 105, units = "mm")
```
