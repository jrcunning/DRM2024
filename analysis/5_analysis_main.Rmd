...
## 1) Fit models
## OLS model
lm_fit <- lm(breadth ~ ngen, data = eds_breadth)

## Right-censored model
library(survival)
cens_fit <- survreg(
  Surv(breadth, event, type = "right") ~ ngen,
  data = eds_breadth,
  dist = "gaussian"
)
...
pred_df <- tibble(
  ngen = seq(min(eds_breadth$ngen, na.rm = TRUE),
             max(eds_breadth$ngen, na.rm = TRUE),
             length.out = 10)
) %>% 
  mutate(
...
fig3 <- ggplot(eds, aes(x = ngen, y = Diff_trunc)) +
  ...
  geom_point(aes(shape = reach), position = pos, alpha = 0.6, size = 3) +
