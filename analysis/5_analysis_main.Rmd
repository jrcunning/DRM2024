...
(ssfig <- sym_shifts %>%
  ggplot(aes(x = med, y = degC)) +
  geom_errorbar(aes(ymin = degC - degC.se, ymax = degC + degC.se)) +
  geom_errorbar(aes(xmin = lo, xmax = hi)) +
  geom_point(aes(color = factor(ngen))) +
  geom_text_repel(aes(label = Species)) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(x = "Shift in symbiont composition (Hel dist)", y = "Change in heat tolerance (°C)")
)


# Fit the model, with intercept zero at density change estimate = 0 and med(Hel dist) = 0
#  Weight observations using the standard error of the degC response
ols_model_adjusted <- lm(degC ~ 0 + estimate + med, data = sym_shifts, weights = 1/degC.se^2)
# Check intercept
predict(ols_model_adjusted, newdata = data.frame(estimate = 0, med = seq(0,1,0.1)))
# Check residuals
#plot(ols_model_adjusted)

# Check model summary
(summary_ols <- summary(ols_model_adjusted))
med_pval <- broom::tidy(ols_model_adjusted)[2,5]

# Extract model coefficients
ols_slope_estimate <- coef(ols_model_adjusted)["estimate"]
ols_slope_helmed <- coef(ols_model_adjusted)["med"]

# Compute contributions of each predictor
sym_shifts$Estimate_Contribution <- ols_slope_estimate * sym_shifts$estimate
sym_shifts$helmed_Contribution <- ols_slope_helmed * sym_shifts$med

# Compute residual variation (unexplained variation)
sym_shifts$Unexplained_Variation <- sym_shifts$degC - (sym_shifts$Estimate_Contribution + sym_shifts$helmed_Contribution)

# Verify that the sum matches observed degC exactly
sym_shifts$Check_Sum <- sym_shifts$Estimate_Contribution + sym_shifts$helmed_Contribution + sym_shifts$Unexplained_Variation
all.equal(sym_shifts$Check_Sum, sym_shifts$degC)  # Should return TRUE

# Plot contributions of each predictor to observed degC values
outres <- sym_shifts %>%
  select("Species", "degC", "degC.se", 
         "Estimate_Contribution", "helmed_Contribution", "Unexplained_Variation") %>%
  pivot_longer(cols = 4:6) %>%
  arrange(degC) %>%
  mutate(Species = fct_reorder(Species, degC))

(decomp <- ggplot(outres, aes(x = Species, y = value, fill = name)) +
  geom_col(position = "stack", alpha = 1) +
  geom_point(aes(y = degC, shape = "Net change (±SE)"), size = 2) +
  geom_errorbar(aes(ymin = degC - degC.se, ymax = degC + degC.se), width = 0) +
  scale_x_discrete(labels = abbrev_names[as.character(outres$Species)]) +
  scale_fill_manual(name = "Modeled contributions", 
                      labels = c("Population size", "hel med", "Unexplained"),
                      values = c("#67a9cf", "#ef8a62", "#C0C0C0")) +
  scale_shape_manual(name = "", values = 16) +
  theme_classic() +
  theme(axis.text.x = element_text(face = "italic", angle = 45, hjust = 1, size = 10),
...
#^^^ one way to calculate. 
#VVV another way. equivalent, but more explicit because ngen_centered = 0 and estimate = 0 are specified in predict
sym_shifts$Adj_degC_Estimate <- predict(ols_model_adjusted, newdata = data.frame(estimate = sym_shifts$estimate, med = 0)) + residuals(ols_model_adjusted)
sym_shifts$Adj_degC_med <- predict(ols_model_adjusted, newdata = data.frame(estimate = 0, med = sym_shifts$med)) + residuals(ols_model_adjusted)
sym_shifts$Residual_Variation <- residuals(ols_model_adjusted)
### We're adding the total residuals for both effect plots. is this correct? or do we need get partial residuals?


# Generate regression lines data using correct centering
estimate_seq <- seq(min(sym_shifts$estimate), max(sym_shifts$estimate), length.out = 100)
...
regression_line_med <- data.frame(
  med = med_seq,
  predicted_degC = ols_slope_helmed * med_seq)

# Plot Effect of Estimate on Adjusted degC (Holding ngen = 1, with residuals)
ggplot(sym_shifts, aes(x = estimate, y = Adj_degC_Estimate)) +
  geom_line(data = regression_line_estimate, aes(x = estimate, y = predicted_degC), 
            color = "#67a9cf", linewidth = 1.5, alpha = 1) +
  geom_point(size = 3, alpha = 0.7, color = "#7F7F7F", stroke = 0) +
  geom_errorbar(aes(ymin = Adj_degC_Estimate - degC.se, ymax = Adj_degC_Estimate + degC.se), 
...
p2 <- ggplot(sym_shifts, aes(x = med, y = Adj_degC_med)) +
  ...
  geom_errorbar(aes(ymin = Adj_degC_med - degC.se, ymax = Adj_degC_med + degC.se), 
                width = 0, alpha = 0.7, lwd = 0.3) +
  annotate(geom = "text", x = -Inf, y = Inf, label = paste("p = ", round(med_pval, 5)), 
           hjust = -0.1, vjust = 1.1) +
  ggrepel::geom_text_repel(aes(label = abbrev_names[as.character(Species)]),
                           fontface = "italic", size = 3, seed = 8) +
  labs(x = "Change in symbiont composition\n(2001-03 to 2019-20; Hellinger distance)", 
       y = "Change in bleaching threshold (°C)\n(adjusted for pop. density change = 0)") +
  scale_size_continuous(range = c(2, 5)) +
  theme_classic() +
...
p1 <- fig +
  ...
  theme_classic() +
  labs(x = "Number of symbiont genera", 
       y = "Change in symbiont composition\n(2001-03 to 2019-20; Hellinger distance)")
...
# NOW DO NGEN AND DENSITY FOR THIS SUBSET OF SPECIES
# STATISTICALLY MODEL DENSITY AND SYMBIONT EFFECTS
...
# Fit the model, with intercept zero at estimate = 0 and ngen = 1
#  Weight observations using the standard error of the degC response
ols_model_adjusted <- lm(degC ~ 0 + estimate + ngen_centered, data = sym_shifts, weights = 1/degC.se^2)
# Check intercept
predict(ols_model_adjusted, newdata = data.frame(estimate = 0, ngen_centered = 0))
# Check residuals
#plot(ols_model_adjusted)
...
#^^^ one way to calculate. 
#VVV another way. equivalent, but more explicit because ngen_centered = 0 and estimate = 0 are specified in predict
sym_shifts$Adj_degC_Estimate2 <- predict(ols_model_adjusted, newdata = data.frame(estimate = sym_shifts$estimate, ngen_centered = 0)) + residuals(ols_model_adjusted)
sym_shifts$Adj_degC_Ngen <- predict(ols_model_adjusted, newdata = data.frame(estimate = 0, ngen_centered = sym_shifts$ngen_centered)) + residuals(ols_model_adjusted)
sym_shifts$Residual_Variation <- residuals(ols_model_adjusted)
### We're adding the total residuals for both effect plots. is this correct? or do we need get partial residuals?
...
# Plot Effect of Ngen on Adjusted degC (Holding estimate = 0, with residuals)
p22 <- ggplot(sym_shifts, aes(x = Jittered_Ngen, y = Adj_degC_Ngen)) +
  geom_line(data = regression_line_ngen, aes(x = ngen, y = predicted_degC), 
            color = "#ef8a62", linewidth = 1.5, alpha = 1) +
  geom_point(size = 3, alpha = 0.7, color = "#7F7F7F", stroke = 0) +
  geom_errorbar(aes(ymin = Adj_degC_Ngen - degC.se, ymax = Adj_degC_Ngen + degC.se), 
...
cowplot::plot_grid(p1, p2, p22, nrow = 1, align = "hv")
```
