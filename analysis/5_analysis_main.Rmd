...
pick_peak_date <- function(df_year,
                           delta = 1.5,          # DHW units; tune if needed
                           minpeakdistance = 7,  # days
                           eps = 1e-4) {

  df_year <- df_year %>% arrange(date)

  x <- df_year$mean_dhw_smooth
  ...
  # candidate local maxima indices
  if (is.null(pk)) {
    p <- which.max(x)
    h <- x[p]
  } else {
    ...
    # keep peaks "close enough" to the annual max
    keep_idx <- peaks_idx[(mx - peaks_h) <= delta]

    if (length(keep_idx) == 0) {
      # fallback: highest peak
      p <- peaks_idx[which.max(peaks_h)]
    } else {
      # key rule: earliest near-max peak
      p <- min(keep_idx)
    }
    h <- x[p]
  }

  # plateau handling: use first day of the plateau block around height h
  plateau <- which(abs(x - h) <= eps)
  if (length(plateau) > 0) {
    runs <- split(plateau, cumsum(c(1, diff(plateau) != 1)))
    block <- runs[sapply(runs, function(r) any(r == p))][[1]]
    p <- min(block)
  }

  df_year[p, , drop = FALSE]
}

peak_dates <- dhw_daily %>%
  group_by(year) %>%
  group_modify(~ pick_peak_date(.x, delta = 1.5)) %>%
  ungroup() %>%
  mutate(
    week  = isoweek(date),
    Week2 = cut(week, breaks = seq(0, 54, by = 2), right = TRUE)
  )
...
nrow(model.frame(dhw.mod))
allg %>% filter(is.na(Subregion))
...
set.seed(1)
boot.outnull <- bootMer(dhw.mod, FUN = ed50s.fun_fast, nsim = 10, re.form = NULL,
...
bleach50
apply(boot.outnull$t, 2, function(x) as.numeric(quantile(x, probs=c(0.025, 0.5, 0.975), na.rm=TRUE)))
apply(boot.outnull$t, 2, function(x) as.numeric(quantile(x, probs=0.5, na.rm=TRUE)))
...
dhw.bleach.plot <- ggplot(as.tibble(dhw.res2), aes(x = dhw, y = prob, group = year)) +
  ...
  geom_ribbon(aes(ymin = asymp.LCL, ymax = asymp.UCL), lwd = 0, alpha = 0.1) +            # WHY NOT SE?
  ...
  geom_text(data = dhw_ranges,
           aes(x = maxdhw, y = c(0.68, 0.58, 0.9, 0.19), label = year),
           hjust = c(0,0,1,0), nudge_y = c(0,0,0.03,-0.05)) +
  ...
  scale_y_continuous(limits = c(-0.025, 0.95), expand = expansion(mult = c(0, 0)), breaks = seq(0, 1, 0.1), 
...
(dhw.bleach.plot +
  annotate("text", x = 14.2, y = 0.04, hjust = 0, size = 3, 
           label = paste0("+", signif(ed50conf$ed50[[6]], 2), " DHW\n", 
                          "= +", signif(finaldegC23$degC, 2), " °C")) +
  annotate("text", x = 18.9, y = 0.04, hjust = 0, size = 3, fontface = "italic",
...
dhw.sp.res <- emmeans(dhw.sp.mod, specs = c("Species", "dhw", "year", "Week2"), type = "response",
                      at = list(dhw = seq(0, 23, 0.1), Week2 = c("(34,36]","(36,38]","(38,40]")), 
...
dhw.sp.res2 <- subset(dhw.sp.res,
  ...
  (year == "2023" & Week2 == "(36,38]" & dhw <= 21.5 & dhw > 1.06)# |
...
ggsave(filename = "figures/FigS4new5.png", plot = dhw.sp.bleach.plot, width = 140, height = 140, units = "mm")
...
sp.boot.out <- bootMer(dhw.sp.mod, FUN = sp.ed50s.fun, nsim = 10, seed = 123, re.form = NULL,
