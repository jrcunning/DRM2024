...
df24 <- df24 %>% filter(Subregion != "Tortugas--Tortugas Bank")   # exclude Tortugas Bank bc not present in other years
...
# SHOULD WE FILTER OUT MARQUESAS TOO?? NOT SURVEYED IN 2014 or 2015....
```
...
allf <- allf %>% filter(Subregion != "Marquesas")
...
# CUMMAXDHW
allg2 <- allgf %>%
  filter(!(dhw < cummaxdhw)) # only keep sites that were before or at the dhw peak, not after

dhw.mod4 <- glmer(cbind(BL, NB) ~ cummaxdhw + year + (cummaxdhw + year | Species) + (year | Subregion),  
                 family = "binomial", data = allg2, 
                 verbose = FALSE, nAGQ = 0, control = glmerControl(optimizer = "nloptwrap"))

hi <- emmeans(dhw.mod4, specs = c("year", "cummaxdhw"), type = "response",
        at = list(cummaxdhw = seq(0,23,0.1)))

dhw.bleach.plot <- ggplot(as.tibble(hi), aes(x = cummaxdhw, y = prob, group = year)) +
  # Add lines and conf intervals for each year
  geom_ribbon(aes(ymin = prob - SE, ymax = prob + SE), lwd = 0, alpha = 0.1) +
  geom_line(aes(color = prob), lwd = 2) +
  #geom_text(data = slice(ed50conf, 1:4), 
