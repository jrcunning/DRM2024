...
sp.degC.w[which(sp.degC.w$Species == "PDIV"), "degC_w.se"] <- 0.07
...
net <- sp.degC.w %>%
  left_join(dplyr::select(as.tibble(densdiffs), Species, estimate, SE)) %>%
...
net_clean <- na.omit(net[, c("Species", "degC_w", "estimate", "ngen", "degC_w.se", "SE", "D")])
...
summary(lm(degC_w ~ estimate + ngen_centered, data = net_clean))
summary(lm(degC_w ~ estimate + ngen_centered, weights = 1/degC_w.se^2,
           data = filter(net_clean, Species != "PDIV")))
...
ols_model_adjusted_factor <- lm(degC ~ estimate + factor(ngen_centered), data = net_clean, weights = 1/degC.se^2)
