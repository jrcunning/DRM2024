...
sp.degC.w[which(sp.degC.w$Species == "PDIV"), "degC_w.se"] <- 0.0475
...
# UPDATE: CONVERT TO DEGC INSIDE BOOTSTRAP?
boot_degC <- sweep(sp.boot.out.24$t, 2, sp_mean_slope$mean_slope, "/")
degC_se <- apply(boot_degC, 2, sd, na.rm = TRUE)
degC_est <- sp.ed50s %>%
  left_join(sp_mean_slope, by = "Species") %>%
  mutate(degC_est = ed50 / mean_slope)
```
