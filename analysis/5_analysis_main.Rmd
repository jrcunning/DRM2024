...
fig <- ggplot(sym_shifts, aes(x = ngen, y = med)) +
...
regression_line_mod_w <- tibble(
  ngen_seq = seq(1, 4, 1),
  predicted_med = coef(mod_w)[1] * ngen_seq - coef(mod_w)[1]
)

p1 <- fig +
  geom_line(data = regression_line_mod_w, aes(x = ngen_seq, y = predicted_med), 
            color = "#ef8a62", linewidth = 1.5, alpha = 1) +
  scale_x_continuous(breaks = seq(1, 4, 1), limits = c(1, 4.2)) +
  coord_cartesian(xlim = c(1, 4.2), ylim = c(0, 0.6)) +
  annotate(geom = "text", x = -Inf, y = Inf, label = paste("p = ", round(mod_w_pval, 5)), 
           hjust = -0.1, vjust = 1.1) +
  theme_classic() +
  labs(x = "Number of symbiont genera", 
...
p2 <- ggplot(sym_shifts, aes(x = med, y = Adj_degC_med)) +
  ...
  scale_size_continuous(range = c(2, 5)) +
  theme_classic() +
  coord_cartesian(xlim = c(0, 0.6), ylim = c(0, 2.5))
