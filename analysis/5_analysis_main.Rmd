...
fig3 <- ggplot(eds, aes(x = Estimate, y = Diff)) + 
...
figS3 <- ggplot(eds, aes(x = ngen, y = Diff)) +
  ...
  geom_line(data = data.frame(x = c(1, 4), 
            ...
            aes(x = x, y = y), color = "black", alpha = 0.3, linewidth = 1) +
...
ggsave(filename = "figures/FigS3.png", plot = figS3, width = 140, height = 120, units = "mm")
```


### Within-species variation (ED90-ED10)
```{r within_spp_bleaching_variability_old, fig.width = 8, fig.height = 5}
...
ngen <- ggplot(out, aes(x = ngen, y = span)) +
  ...
  stat_smooth(geom = "line", method = "glm", se = FALSE) +
  ggpubr::stat_cor() +
  ggrepel::geom_text_repel(aes(label = abbrev_names[as.character(Species)]),
                           fontface = "italic", size = 4, lineheight = 0.75) +
