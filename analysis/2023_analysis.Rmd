...
# Who did the most surveys?
teams <- df23 %>%
  distinct(Site, Team, Date) %>%
  count(Team) %>%
  arrange(-n)

teams
# Shedd did 76 surveys in total
sites <- df23 %>%
  distinct(Site, Subregion, Team, Date, dhw)
sites %>%
  filter(dhw > 12)

drto <- sites %>% filter(Subregion == "Tortugas--Dry Tortugas NP")
...
surveys_by_date <- ggplot(surveys, aes(x = Date, y = Subregion, fill = Subregion)) +
  ggridges::geom_density_ridges(alpha = 0.6, stat = "binline", bins = 20) +
  labs(x = "Date", y = "") +
  theme(legend.position = "none")

ggplot(surveys, aes(x = dhw, fill = Subregion)) +
...
m <- glm(Bleaching2 > 1 ~ Species * dhw + EndDepth + Week2, family = "binomial", data = df23.abund)
...
res <- emmeans(m, specs = c("dhw"), rg.limit = 20000, type = "response",
               at = list(Week2 = factor("(33,35]"), Depth = 23, dhw = seq(0, 20, 0.1)))
...
bleachprob23 <- as.tibble(res)
saveRDS(bleachprob23, file = "output/bleachprob23.rds")
```
...
ggplot(df23.abund, aes(x = cut(dhw, 10), fill = Bleaching3)) +
...
m <- glm(Bleaching2 > 1 ~ Species * dhw + Week2, family = "binomial", data = df23.abund.ss)
summary(m)

# Get fitted values across dhw [normalize to Aug13-26 window of bleaching rates]
res <- emmeans(m, specs = c("Species", "dhw"), rg.limit = 20000, type = "response",
               at = list(Week2 = factor("(33,35]"), dhw = seq(0, 20, 0.1)))
...
sppbleachprob23 <- as.tibble(res)
saveRDS(sppbleachprob23, file = "output/sppbleachprob23.rds")
```
...
# Fit regression for PB or worse
m <- glm(Bleaching2 > 1 ~ Species * dhw.bin + Week2, family = "binomial", data = df23.abund.ss)
