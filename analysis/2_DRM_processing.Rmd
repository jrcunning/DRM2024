...
```{r}
# List all valid DRM species codes
species_codes <- c("AAGA","ACER","ACRO","AFRA","AGAR","AGRA","AHUM","ALAM",
                   "APAL","APRO","ATEN","AUND","CARB","CNAT","DCYL","DIPL",
                   "DLAB","DSTO","EFAS","FFRA","HCUC","IRIG","ISIN","ISOP",
                   "MADR","MALC","MALI","MANG","MARE","MAUR","MCAV","MCOM",
                   "MDEC","MEAN","MFER","MFOR","MILL","MJAC","MLAM","MMEA",
                   "MPHA","MSEN","MYCE","OANN","OCUL","ODIF","OFAV","OFRA",
                   "ORBI","OVAR","PAME","PAST","PBRA","PCLI","PDIV","PFUR",
                   "PORI","PPOR","PSEU","PSTR","SBOU","SCOL","SCUB","SHYA",
                   "SIDE","SINT","SLAC","SOLE","SRAD","SSID","SWEL","UNKN")
```
...
df14 <- df14 %>%
  mutate(Species = factor(Species, levels = species_codes))
...
dhw <- read_csv("data/processed/2014_dhw_calc_site_surveyed.csv") %>%
  mutate(across(where(is.character), as_factor)) %>%
  #rename(Date = date) %>%
...
dhw <- read_csv("data/processed/2015_dhw_calc_site_surveyed.csv") %>%
  mutate(across(where(is.character), as_factor)) %>%
  #rename(Date = date) %>%
  dplyr::select(Site, Date, dhw)
...
dhw <- read_csv("data/processed/2023_dhw_calc_site_surveyed.csv") %>%
  mutate(across(where(is.character), as_factor)) %>%
  #rename(Date = date) %>%
  dplyr::select(Site, Date, dhw)
...
t3t4 <- readxl::read_xlsx("data/drm/2024ANU_RawCoralDataTransect3and4_QAQC.xlsx") %>%
  ...
  drop_na(Species)    # DROP T3/T4 WITH NO CORALS OBSERVED (Species == NA)
nrow(t3t4)
t3t4 %>% count(Bleaching)
t3t4 <- t3t4 %>%
  mutate(Bleaching = case_match(
    Bleaching, 
    "Not Bleached" ~ NA,
...
df24 %>% count(Bleaching2)
df24 %>% filter(grepl("TRS", `Other Conditions`)) %>% 
  dplyr::select(Date, Species, Bleaching, Old, Recent, `Other Conditions`) %>%
  print(n = nrow(.))
df24 %>% filter(is.na(Bleaching2))


# There are some corals with TRS but no BL condition recorded, treat these as not bleached.
df24 <- df24 %>%
...
dhw <- read_csv("data/processed/2024_dhw_calc_site_surveyed.csv") %>%
  mutate(across(where(is.character), as_factor)) %>%
  #rename(Date = date) %>%
  dplyr::select(Site, Date, dhw)
...
df <- read_csv("data/drm/FRRP_EXPORT.csv") %>%
  mutate(across(where(is.character), as_factor),
         Date = as_date(as.character(Date), format = "%m/%d/%Y %H:%M:%S")) %>%
  rename(Depth = Enddepth, TLRate = Tlrate,
         `Disease Conditions` = Disease_Conditions, 
...
# Change Deerfield to either Broward or South Palm Beach based on Lat = 26.32825
df[df$Subregion == "Deerfield" & df$Latitude > 26.32825, "Subregion"] <- "South Palm Beach"
df[df$Subregion == "Deerfield" & df$Latitude < 26.32825, "Subregion"] <- "Broward-Miami"
...
# Are there any dead corals?
dead <- df %>% filter(Old + Recent >= 100)
df <- anti_join(df, dead)
...
pctBL <- df %>%
  ...
  group_by(year, Site) %>%
  summarize(pctBL = sum(Bleaching2 >= 2) / n()) %>%
  ungroup()

ggplot(pctBL, aes(x = factor(year), y = pctBL)) + geom_boxplot()

pctBL %>%
  group_by(year) %>%
  summarize(p = quantile(pctBL, 0.9, na.rm = TRUE))
```
