...
df24 <- filter(df, year(Date) == "2024") %>% droplevels()
# How many corals observed?
nrow(df24)
...
dead
# There are 8 recently dead corals -- they have OUK, OGI, or SC conditions -- remove these from dataset
df24 <- anti_join(df24, dead)

# Create ordinal variables and factors for bleaching severity
df24 <- df24 %>%
  mutate(Bleaching2 = case_when((is.na(Bleaching) & !grepl("TRS", `Other_Conditions`)) ~ 0,
                                Bleaching == "P" ~ 1,
                                Bleaching == "PB" ~ 2,
                                Bleaching == "BL" ~ 3,
                                ((Old + Recent) >= 100 & grepl("TRS", `Other_Conditions`)) ~ 4),
         Bleaching3 = factor(case_match(Bleaching2, 
                                 0 ~ "Healthy",
                                 1 ~ "Pale",
                                 2 ~ "Partially Bleached",
...
df24 %>% filter(grepl("TRS", Other_Conditions))
