...
df15[(df15$Old + df15$Recent) >= 100 & !grepl("TRS", df15$`Other Conditions`), "Other Conditions"] <- "TRS"
...
DHWs in DRTO reached ~6.
DHWs in the Lower Keys reach ~8
DHWs in the Middle Keys reached ~6
DHWs in the Upper Keys reached ~5.
DHWs in Biscayne reach 2.5.
DHWs in Miami and north were <1
...
mod <- glm((Bleaching2 > 1) ~ Week:Subregion, family = "binomial", data = df15)
...
mod <- glm((Bleaching2 > 1) ~ Subregion + Week, family = "binomial", data = df15)
...
m <- polr(Bleaching3 ~ dhw * Depth * Species, data = df15.abund, Hess = TRUE, model = TRUE)
...
mod <- glmer((Bleaching2 > 1) ~ Subregion * Species + (1|Week), 
             family = "binomial", data = df15.abund.ss,
...
m <- MASS::polr(Bleaching3 ~ Species * dhw, data = df15.abund, Hess = TRUE, model = TRUE)
## Does this need random factors? Cannot get clmm to work...

# Get fitted values
res <- emmeans(m, specs = c("dhw", "Bleaching3"), by = "Species", mode = "prob", at = list(dhw = seq(0, 10, 1)))
...
mod <- glmer((Bleaching2 > 1) ~ log(Width) * Species * dhw + (1|Site) + (1|Week), 
             family = "binomial", data = filter(df15.abund, dhw > 10), 
             nAGQ = 0, control = glmerControl(optimizer = "nloptwrap"))
#plot(mod, which = 1)
...
mod <- glm((Bleaching2 > 1) ~ Subregion * Width + dhw, 
             family = "binomial", data = filter(df15.abund, dhw > 10, Species == "MCAV"))
plot(mod, which = 1)
anova(mod)
res <- broom::augment(mod) %>%
  bind_cols(filter(df15.abund, dhw > 10, Species == "MCAV"))
...
mcav <- df15.abund %>% filter(Species == "DLAB")
...
hi <- df15 %>% filter(Species == "DLAB", Width < 7)

range(mcav$Width)


### all species?
df15ss3 <- filter(df15ss2, Species %in% sppcounts$Species[1:10])
mod <- glmer(BL ~ Species * Width + dhw + (1|Site), family = "binomial", data = df15ss3,
             nAGQ = 0, control = glmerControl(optimizer = "nloptwrap"))
anova(mod)
res <- emmeans(mod, specs = c("Width", "Species", "dhw"), 
               at = list(Width = seq(4,500,2), dhw = 15), type = "response")
sizeranges <- df15ss3 %>% group_by(Species) %>% summarize(min = min(Width), max = max(Width))
sizeranges
res
...
ggplot(df15.abund, aes(y = Subregion, x = log(Width))) +
  geom_boxplot() +
  facet_wrap(~Species)
mod <- lm(log(Width) ~ Subregion * Species, data = df15.abund)
anova(mod)
...
# miami <- filter(df15, Subregion == "Broward-Miami")
...
# df15 %>% filter(Site == "Y3042", Species == "PAST") %>% print(n = nrow(.))
...
site.resid <- tibble(df15.abund, .resid = residuals(mod, type = "response")) %>% 
  group_by(Site, Longitude, Latitude) %>%
  summarize(meanResid = mean(.resid)) %>%
  arrange(meanResid)
