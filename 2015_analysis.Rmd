...
# Assign dead corals NOT in those regions TRS (dead from belaching)
...
dhw %>%
  ...
  geom_line(aes(group = Site, color = Subregion), alpha = 0.4) +
...
# Remove...
# df15 <- df15 %>% filter(!Subregion %in% c("South Palm Beach", "North Palm Beach", "Martin"))
# 
# surveys <- df15 %>%
#   distinct(Subregion, Date, Site, dhw)
```
...
res <- as.tibble(emmeans(mod, specs = c("Week", "Subregion"), type = "response")) %>% 
  drop_na() %>%
  mutate(Date = as.Date("2015-01-01") + ((as.numeric(as.character(Week)) - 1) * 7))
...
regional15 <- as.tibble(res) %>% full_join(maxmeanDHW) %>%
  mutate(group = "allcorals")
...
m <- MASS::polr(Bleaching3 ~ Species * dhw, data = df15.abund, Hess = TRUE)
