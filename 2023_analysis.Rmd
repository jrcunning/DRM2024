...
# dead <- df23 %>%
#   filter((Old + Recent) >= 100 & !grepl("TRS", df23$`Other Conditions`))
# df23[(df23$Old + df23$Recent) >= 100 & !grepl("TRS", df23$`Other Conditions`), "Other Conditions"] <- "TRS"
...
# df23.abund %>% filter(is.na(Bleaching2))
...
mod <- glmer(cbind(bl, nb) ~ Subregion + (1|Week) + (1|Species), family = "binomial", data = summ)
res <- emmeans(mod, specs = c("Subregion"), type = "response")
regional23 <- as.tibble(res) %>% full_join(maxmeanDHW) %>%
  mutate(group = "allcorals")

ggplot(regional23, aes(y = Subregion)) +
  geom_point(aes(x = prob)) +
  geom_path(aes(x = prob, group = group)) +
  geom_point(aes(x = maxmeanDHW / 25), color = "red") +
  geom_path(aes(x = maxmeanDHW / 25, group = group), color = "red") +
  scale_x_continuous(limits = c(0, 1), expand = c(0, 0), breaks = seq(0, 1, 0.1),
                     name = "Predicted % of 'avg coral' PB or worse",
                     sec.axis = sec_axis(~.*25, name = "Max. Mean DHW")) +
  theme(axis.text.x.top = element_text(color = "red"),
        axis.title.x.top = element_text(color = "red")) +
  labs(y = "")
