...
# Percent of surveyed colonies with at least partial bleaching by Subregion and Week
df23 %>%
  group_by(Week, Subregion) %>%
  summarize(pctbl = sum(Bleaching2 > 1) / n()) %>%
  ggplot(aes(x = Week, y = pctbl, group = Subregion, color = Subregion)) +
  geom_point() + 
  geom_line() +
...
## Use 2-Week periods in model, and predict based on weeks 35-36 (Week2 = "(34,36]" => (August 27-September 9))) = peak of heat stress, before too much mortality
mod <- glm(cbind(bl, nb) ~ Subregion + Species + Week2, family = "binomial", data = summ)
res <- emmeans(mod, specs = c("Subregion"), at = list(Week2 = factor("(34,36]")), type = "response")
# Join predicted bleaching prevalence for Aug27-Sep9 with average DHWs during same dates
mymeandhw <- dhw %>%
  left_join(distinct(df23, Site, Subregion)) %>%
  filter(Date >= "2023-08-27", Date <= "2023-09-09") %>%
  group_by(Subregion) %>%
  summarize(meanDHW = mean(dhw)) %>%
  drop_na()

regional23 <- as.tibble(res) %>% full_join(mymeandhw) %>%
  mutate(group = "allcorals")
...
ggplot(regional23, aes(y = Subregion)) +
  ...
  scale_x_continuous(limits = c(0, 1), expand = c(0, 0), breaks = seq(0, 1, 0.1),
                     name = "Predicted % of 'avg coral' PB or worse [Aug27-Sep6]",
                     sec.axis = sec_axis(~.*25, name = "Mean DHW [Aug27-Sep6]")) +
  theme(axis.text.x.top = element_text(color = "red"),
        axis.title.x.top = element_text(color = "red")) +
  labs(y = "")
...
m <- polr(Bleaching3 ~ dhw * EndDepth * Species, data = df23.abund, Hess = TRUE, model = TRUE)
...
weeks <- df23.abund %>%
  group_by(Week) %>%
  summarize(pctbl = sum(Bleaching2 > 1, na.rm = T) / n())
ggplot(weeks, aes(x = Week, y = pctbl)) + geom_point()


# Fit ordinal regression model for each species as a function of DHW
m <- MASS::polr(Bleaching3 ~ Species * dhw + Week, data = df23.abund, Hess = TRUE, model = TRUE)
## Does this need random factors? Cannot get clmm to work...

# Get fitted values
# Choosing at which week makes a big difference in overall bleaching rates
res <- emmeans(m, specs = c("dhw", "Bleaching3"), by = "Species", mode = "prob", at = list(dhw = seq(0, 23, 1),
                                                                                           Week = factor(35)))
weeks2<-as.tibble(emmeans(m, specs = "Week"))
ggplot(weeks2, aes(x = Week, y = emmean)) + geom_point()
