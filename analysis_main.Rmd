...
(dens_sym_viz <- net %>%
  ...
  scale_color_brewer(name = "Symbiont Genera",
...
(decomp <- ggplot(out, aes(x = Species, y = value, fill = name)) +
  ...
  geom_point(aes(y = degC, shape = "Net observed value"), size = 2) +
  ...
  theme(axis.text.x = element_text(face = "italic", angle = 45, hjust = 1, size = 7),
        ...
        legend.spacing.y = unit(0, "cm")) +  # Reduce vertical space between legend items +
...
ps <- cowplot::plot_grid(p1, p2, nrow = 1)

cowplot::plot_grid(ps, decomp, nrow = 2)
```
