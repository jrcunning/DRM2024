...
### ∆°C vs. density changes and symbiont diversity
...
(dens_sym_viz <- net %>%
  ...
  scale_color_discrete(name = "Symbiont Genera") +
  labs(x = "Change in abundance (2014-2023)", 
       y = "Change in bleaching threshold (°C)") +
  theme_classic() +
  theme())

ggsave(filename = "figures/Fig5.png", plot = dens_sym_viz,
...
out <- net_clean %>%
  select("Species", "degC", "Estimate_Contribution", "Ngen_Contribution", "Unexplained_Variation") %>%
...
(decomp <- ggplot(out, aes(x = Species, y = value, fill = name)) +
  geom_col(position = "stack") +
  geom_point(aes(y = degC, shape = "Net observed\nvalue"), size = 2) +
  scale_x_discrete(labels = abbrev_names[as.character(out$Species)]) +
  scale_fill_discrete(name = "Modeled contributions", 
                      labels = c("Population size", "Symbiont genera", "Unexplained")) +
  scale_shape_manual(name = "", values = 16) +
  theme_classic() +
  theme(axis.text.x = element_text(face = "italic", angle = 45, hjust = 1, size = 7),
        ...
        legend.spacing.y = unit(0.3, "cm")) +  # Reduce vertical space between legend items +
  labs(x = "", y = "Change in heat tolerance (°C) 2014-2023") +
  guides(fill = guide_legend(ncol = 1, override.aes = list(shape = NA)),
         shape = guide_legend(order = 1, label.theme = element_text(size = 8))))

ggsave("figures/FigS72.png", plot = decomp, width = 120, height = 90, units = "mm")
