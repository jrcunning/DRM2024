...
# Combine tolerance range (span10-90) with symbiont data from Florida
library(gsheet)
...
```{r spp_degC_syms}
# In symbiont dataset, aggregate coral taxa to match aggregation in DRM dataset
symsumm.ag <- sym0  %>%
  filter(location == "FL") %>%
  mutate(Species = case_when(Species %in% c("AFRA", "AGAR", "AGRA", "AHUM", "ALAM") ~ "AGAR",
                             Species %in% c("MALI", "MLAM", "MYCE", "MFER") ~ "MYCE",
...
mod <- lm(degC ~ estimate, data = filter(net, ngen > 1))
anova(mod)

mod <- lm(degC ~ ngen, data = filter(net, ngen > 1))
anova(mod)


mod <- lm(degC ~ estimate + ngen, data = filter(net, ngen > 1))
anova(mod)

ggplot(filter(net, ngen > 1), aes(x = estimate, y = ngen)) +
  geom_point()
```
