...
# Distance from 10% to 90% bleaching (90% to 10% not bleached) by Species
span1090 <- drc.res %>%
  group_by(Species) %>%
...
# Plot relationship between number of symbiont genera and tolerance range
span1090 %>%
  full_join(symsumm) %>%
...
test <- span1090 %>% full_join(symsumm)
mod <- lm(span ~ ngen, data = test)
anova(mod)
```
...
counts <- all0 %>% count(Species, year)
...
mod <- lm(degC ~ ngen + D, data = filter(symsumm.ag, ngen > 1))
anova(mod)
emmeans(mod, specs = c("ngen", "D"), at = list(ngen = c(1,2,3,4)))
mod <- lm(degC ~ factor(ngen) + D, data = symsumm.ag)
anova(mod)
...
# Make prettier plot
library(ggrepel)
ggplot(symsumm.ag, aes(x = ngen, y = degC, color = D)) +
...
# TRY REDOING DENSDIFFS WITH ORBICELLA SPP SEPARATE - not better, go back to aggregated ORBI
...
# Test ngen syms AND densdiffs together?
# Combine net changes in heat tolerance and density across species
net <- findat %>%
  left_join(select(as.tibble(densdiffs), Species, estimate, SE)) %>%
  drop_na() %>%
...
mod <- lm(degC ~ estimate, data = filter(net, ngen > 0))
anova(mod)

mod <- lm(degC ~ ngen, data = filter(net, ngen > 0))
anova(mod)


mod <- lm(degC ~ estimate + ngen, data = filter(net, ngen > 0))
anova(mod)

ggplot(filter(net, ngen > 0), aes(x = estimate, y = ngen)) +
  geom_point()

emm_ngen <- emmeans(mod, ~ ngen)
adj_values <- as.data.frame(emmeans(mod, ~ ngen))

net_filtered <- filter(net, ngen > 0)
residuals_adj <- residuals(lm(degC ~ estimate, data = net_filtered))

ggplot(net_filtered, aes(x = ngen, y = residuals_adj)) +
  geom_point() +
  geom_label_repel(aes(label = Species)) +
  geom_smooth(method = "lm", se = TRUE) +
  labs(x = "ngen", y = "Residual degC (adjusted for estimate)", 
       title = "Relationship between ngen and degC after removing estimate effect") +
  theme_minimal()
```
