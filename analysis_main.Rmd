...
### After this commit, I'm trimming this document down to include only the analyses that are part of the manuscript in preparation.
...
surveys.by.dhw2 <- surveys.by.dhw +
...
ggsave(filename = "output/surveys.by.dhw2.png", plot = surveys.by.dhw2, width = 183, height = 90, units = "mm")
```

```{r}
ggplot(df23, aes(x = dhw, fill = Subregion)) +
...
<<<<<<< HEAD
# Shayle symbiont review - updated
syms <- read_csv("symb.genera.20241002.xlsx - Sheet1.csv") %>%
  full_join(span1090) %>%
  mutate(ngen = nchar(Florida)) %>%
  mutate(D = grepl("D", Florida)) %>%
  select(Species, Florida, ngen, D, span)

syms %>%
  ggplot(aes(x = ngen, y = span)) +
  geom_label(aes(label = Species, color = D)) +
  geom_smooth(se = F)

mod <- lm(span ~ ngen, data = syms)
anova(mod)
mod <- lm(span ~ factor(ngen), data = syms)
anova(mod)
=======

# Shayle symbiont review
syms0 <- read_csv("data/symb.genera.20241002.xlsx - Sheet1.csv")
syms <- syms0 %>%
  mutate(ngen = nchar(Florida)) %>%
  dplyr::select(Species, syms = Florida, ngen) %>%
  full_join(span1090)

ggplot(syms, aes(x = ngen, y = span)) +
  geom_point() +
...
mod <- lm(span ~ ngen, data = syms)
anova(mod)
>>>>>>> f54138e6ce907766fa00ed9e8b4e53ebff41b336
emms <- emmeans(mod, specs = "ngen")
pairs(emms)
...
slopes <- as.tibble(emtrends(mod, specs = "Species", var = "year")) %>%
  ...
  dplyr::select(Species, slope, slope.SE)
nets <- sp.ed50conf %>%
  filter(year == "2023-2014") %>%
...
fin <- out %>%
  ...
  dplyr::select(Species, degC.dec = `2014-2023`, degC.dec.se)
...
###########
syms <- syms %>% add_row(Species = "ORBI", Florida = "ABCD", ngen = 4, D = TRUE, span = NA)
findat %>% left_join(syms) %>%
  ggplot(aes(x = ngen, y = degC.dec)) +
  geom_label(aes(label = Species, color = D))

finsym <- findat %>%
  left_join(syms) %>%
  mutate(D = grepl("D", syms))
mod <- lm(degC.dec ~ ngen + D, data = filter(finsym, ngen > 1))
anova(mod)
emmeans(mod, specs = c("ngen", "D"), at = list(ngen = c(1,2,3,4)))
mod <- lm(degC.dec ~ factor(ngen) + D, data = finsym)
anova(mod)
emms <- emmeans(mod, specs = "ngen")
pairs(emms)

library(ggrepel)
ggplot(finsym, aes(x = ngen, y = degC.dec, color = D)) +
  geom_point(size = 5, alpha = 0.6) +
  geom_text_repel(aes(label = abbrev_names[Species]), fontface = 3) +
  
  geom_smooth(se = F, color = "gray40") +
  theme_classic() +
  labs(x = "Number of symbiont genera", y = "Change in heat tolerance (°C/decade)")
