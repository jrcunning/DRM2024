...
mod <- lm(degC ~ estimate + ngen, data = net)
anova(mod) # Both are significant


# Partial residuals plots
residuals_est <- residuals(lm(degC ~ estimate, data = net))
residuals_ngen <- residuals(lm(degC ~ ngen, data = net))

ggplot(net, aes(x = ngen, y = residuals_est)) +
  geom_point() +
  geom_label_repel(aes(label = Species)) +
  geom_smooth(method = "lm", se = TRUE) +
  labs(x = "Number of symbiont genera", y = "Residual degC (adjusted for estimate)") +
  theme_classic()


ggplot(drop_na(net), aes(x = estimate, y = residuals_ngen)) +
  geom_point() +
  geom_label_repel(aes(label = Species)) +
  geom_smooth(method = "lm", se = TRUE) +
  labs(x = "Change in abundance (2014-2023)", y = "Residual degC (adjusted for ngen)") +
  theme_classic()


net$partial_res_ngen <- residuals(mod) + coef(mod)["ngen"] * net$ngen

ggplot(net, aes(x = ngen, y = partial_res_ngen)) +
  geom_point() +
  geom_label_repel(aes(label = Species)) +
  geom_smooth(method = "lm", se = TRUE) +
  labs(x = "ngen", y = "Adjusted degC") +
  theme_minimal()
```
