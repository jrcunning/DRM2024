...
(dhw.bleach.plot <- ggplot(as.tibble(dhw.res2), aes(x = dhw, y = prob, group = year)) +
  ...
  theme_classic() +
  theme(legend.position = "none") +
  labs(x = "Degree Heating Weeks (°C-weeks)", y = "Probability of bleaching") +
...
(tol_dens_plot2 <- net %>%
  ...
  ggplot(aes(x = estimate, y = degC.dec)) + 
  geom_errorbar(aes(ymin = degC.dec - degC.dec.se * degC.per.DHW, 
                    ymax = degC.dec + degC.dec.se * degC.per.DHW), width = 0, alpha = 0.7) +
  geom_errorbar(aes(xmin = estimate - SE, xmax = estimate + SE), width = 0, alpha = 0.7) +
  geom_point(aes(size = (n_symbiont_genera)^2), alpha = 0.7) +
