...
# How were 2014 surveys distributed across time and space?
...
alldhwplot <- dhw %>%
  left_join(distinct(df140, Site, Subregion)) %>%
...
DHWs in DRTO reached ~3-4.
DHWs in the Lower and Middle Keys reach ~6-8
DHWs in the Middle Keys reached ~6-8
DHWs in the Upper Keys reached ~4-6.
DHWs in Biscayne reach 2-3.
DHWs in Miami and Broward was 1-1.5.
...
## Use 2-Week periods in model, and predict based on weeks 37-38 (Week2 = "(36,38]" => (September 8-21))) = peak of heat stress, before too much mortality
...
# Join predicted bleaching prevalence for Sep8-21 with average DHWs during same dates
mymaxdhw <- dhw %>%
  left_join(distinct(df140, Site, Subregion)) %>%
  filter(Date >= "2014-09-08", Date <= "2014-09-21") %>%
  group_by(Subregion, Date) %>%
  summarize(meanDHW = mean(dhw)) %>%
  group_by(Subregion) %>%
...
regional14 <- as.tibble(res) %>% full_join(mymaxdhw) %>%
  mutate(group = "allcorals")
...
ggplot(regional14, aes(y = Subregion)) +
  ...
  scale_x_continuous(limits = c(0, 1), expand = c(0, 0), breaks = seq(0, 1, 0.1),
                     name = "Predicted % of 'avg coral' PB or worse [Sep8-21]",
                     sec.axis = sec_axis(~.*25, name = "Max DHW [Sep8-21]")) +
  theme(axis.text.x.top = element_text(color = "red"),
        axis.title.x.top = element_text(color = "red")) +
  labs(y = "")
...
m <- polr(Bleaching3 ~ dhw * Species + Depth + Week2, data = df14.abund, Hess = TRUE, model = TRUE)
...
m <- glm(Bleaching2 > 1 ~ Species * dhw + Depth + Week2, family = "binomial", data = df14.abund)
...
bleachprob14 <- as.tibble(res)
saveRDS(bleachprob14, file = "output/bleachprob14.rds")
```
...
m <- MASS::polr(Bleaching3 ~ Species * dhw + Week2, data = df14.abund, Hess = TRUE, model = TRUE)
...
ggplot(df14.abund, aes(x = cut(dhw, 10), fill = Bleaching3)) +
  geom_bar(position = "fill") +
...
df14.abund.ss <- filter(df14.abund, Species %in% incl_spp$Species)

# Fit regression for PB or worse
m <- glm(Bleaching2 > 1 ~ Species * dhw + Week2, family = "binomial", data = df14.abund.ss)
...
sppbleachprob14 <- as.tibble(res)
saveRDS(sppbleachprob14, file = "output/sppbleachprob14.rds")
```

```{r BINS}
# Species-specific bleaching susceptibility w/increasing DHW
## Plot raw data/bleaching categoties by binned.dhw
ggplot(df14.abund, aes(x = cut(dhw, breaks = seq(0,9,3)), fill = Bleaching3)) +
  geom_bar(position = "fill") +
  geom_text(stat = "count", aes(label = after_stat(count)), vjust = "inward", position = "fill", size = 2) +
  facet_wrap(~Species, scales = "free_y")
...
# Fit regression for PB or worse
m <- glm(Bleaching2 > 1 ~ Species * dhw.bin + Week2, family = "binomial", data = df14.abund.ss)
...
df14.abund <- df14.abund %>%
  mutate(Width2 = case_when(Width >= 4 ~ Width,
                            Width < 4 ~ 4))

# Fit model
mod <- glm(Bleaching2 > 1 ~ Species * Width2 * dhw + Week2, family = "binomial", data = df14.abund)
#plot(mod, which = 1)
